...
void run_ana_46Ar(TString FileNameHead = "run_", Int_t num_ev = 100000000, Int_t file_ini = 50, Int_t file_end = 154,
                  Int_t runnum = 250, TString file = "../Kinematics/Decay_kinematics/Kine.txt")
{
  ...
  TCanvas *c3 = new TCanvas("c3","c3",200,10,700,700);
   ...
   for (Int_t i = file_ini; i <= file_end; i++) {
      ...
      if (file->IsOpen()) {
         ...
         while (Reader1.Next() && nEve < num_ev) {
            ...
            if (tEnerMin == tEnerMinBuff)
               continue;
            // std::cout<<tEnerMin<<"  "<<tNormChi2<<std::endl;
            if (nEve % 1000 == 0)
               std::cout << " Event : " << nEve << "/" << nEvents << std::endl;
            if (tNormChi2 < 4)
               EAKine->Fill(tThetaMin, tEnerMin);
   ...
   }else if(kineStr3->fail()) std::cout<<" Warning : No Kinematics file found for this reaction! Please run the macro on
   $SIMPATH/macro/Kinematics/Decay_kinematics/Mainrel.cxx"<<std::endl;
