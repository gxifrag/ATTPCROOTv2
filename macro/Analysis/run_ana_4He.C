...
void run_ana_4He(TString FileNameHead = "run_", Int_t num_ev = 100000000, Int_t file_ini = 80, Int_t file_end = 84,
                 TString file = "../Kinematics/Decay_kinematics/Kine.txt")
{
   ...
   TH2F *AnglePhi_vs_AngleSum =
      new TH2F("AnglePhi_vs_AngleSum", "AnglePhi_vs_AngleSum", 1000, 0, 180.0, 1000, 0, 180.0);
   ...
   for (Int_t i = file_ini; i <= file_end; i++) {
      if (i < 10)
         FileNameHead_num.Form("_000%i", i);
      else if (i < 100)
         FileNameHead_num.Form("_00%i", i);
      else if (i < 1000)
         FileNameHead_num.Form("_0%i", i);
      ...
      if (file->IsOpen()) {
         ...
         while (Reader1.Next() && nEve < num_ev) {
            if (nEve % 1000 == 0)
               std::cout << " Event number : " << nEve << std::endl;
            nEve++;
            ...
            if (tracks.size() == 2) {
               ...
               if ((track_r.GetQuadrant() == 0 && track_s.GetQuadrant() == 2) ||
                   (track_r.GetQuadrant() == 2 && track_s.GetQuadrant() == 0)) {
                  ...
                  AngleSumMC->Fill(track_s.FitParameters.sThetaMin * rad2deg +
                                   track_r.FitParameters.sThetaMin * rad2deg);
                  Range_vs_AngleH->Fill(track_r.GetGeoTheta() * rad2deg,
                                        track_r.GetLinearRange(fTrackingEvent->GetGeoVertex()));
                  Range_vs_AngleH->Fill(track_s.GetGeoTheta() * rad2deg,
                                        track_s.GetLinearRange(fTrackingEvent->GetGeoVertex()));
                  ...
                  VertexEvsEnersum->Fill(track_r.GetGeoEnergy() + track_s.GetGeoEnergy() + Eoffset * 2.0,
                                         fTrackingEvent->GetVertexEnergy());
                  AnglevsEnersum->Fill(track_r.GetGeoTheta() * rad2deg,
                                       track_r.GetGeoEnergy() / track_s.GetGeoEnergy());
                  AnglevsEnersumMC->Fill(track_r.FitParameters.sThetaMin * rad2deg,
                                         track_r.FitParameters.sEnerMin / track_s.FitParameters.sEnerMin);
                  ...
                  VertexXvsEnersum->Fill(fTrackingEvent->GetVertex(),
                                         track_r.GetGeoEnergy() + track_s.GetGeoEnergy() + Eoffset * 2.0);
                  Chi2MC->Fill(track_s.FitParameters.sChi2Min);
                  VertexEvsEnersumMC->Fill(track_r.FitParameters.sEnerMin + track_s.FitParameters.sEnerMin +
                                              Eoffset * 2.0,
                                           track_r.FitParameters.sVertexEner);
                  Range1_vs_Range2->Fill(track_r.GetLinearRange(fTrackingEvent->GetGeoVertex()),
                                         track_s.GetLinearRange(fTrackingEvent->GetGeoVertex()));
                  E1vsE2->Fill(track_r.GetGeoEnergy() + Eoffset, track_s.GetGeoEnergy() + Eoffset);

                  AnglePhi_vs_AngleSum->Fill(track_s.GetGeoTheta() * rad2deg,
                                             track_s.GetGeoTheta() * rad2deg + track_r.GetGeoTheta() * rad2deg);

                  kine_file << track_r.GetGeoTheta() * rad2deg << " " << track_s.GetGeoTheta() * rad2deg << " "
                            << fTrackingEvent->GetVertex() << std::endl;
               ...
               } else if ((track_r.GetQuadrant() == 1 && track_s.GetQuadrant() == 3) ||
                          (track_r.GetQuadrant() == 3 && track_s.GetQuadrant() == 1)) {
   ...
   for (Int_t i = 1; i < ang_distr->GetSize(); i++) {
      ...
      if (ang_distr->GetBinCenter(i) > 60.0)
         xsection = ang_distr->GetBinContent(i) / (Nthickness * Nevents * Nthicknorm * sca_factor);
      else
         xsection = ang_distr->GetBinContent(i) / 5.0 / (Nthickness * Nevents * Nthicknorm * sca_factor);
      ang_distr_graph->SetPoint(i, ang_distr->GetBinCenter(i), xsection);
      ang_distr_graph->SetPointError(i, 2.5, xsection * 0.1);
   }

   Double_t exp_ang[13] = {30.5962270477, 35.1056211632, 40.2635563711, 45.211579425,  50.4941499742,
                           54.821182906,  60.1321790594, 65.4626358188, 70.8049001368, 75.6184482171,
                           80.2196789,    84.9295828543, 90.1793546295};
   Double_t exp_xs[13] = {742.0481512359 / 1000.0, 495.5360607239 / 1000.0, 259.1898861892 / 1000.0,
                          112.3365404207 / 1000.0, 27.7036797969 / 1000.0,  4.6908988192 / 1000.0,
                          34.0926846539 / 1000.0,  107.3499120953 / 1000.0, 203.4859477312 / 1000.0,
                          285.5202563417 / 1000.0, 368.126863799 / 1000.0,  444.4166944783 / 1000.0,
                          448.7976365004 / 1000.0};
