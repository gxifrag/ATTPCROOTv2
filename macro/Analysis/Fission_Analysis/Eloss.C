...
void Eloss()
{
   ...
   if (ElossData->fail()) {
      std::cout << cRED << " =  No file found! Please, check the path. Current :" << fFileNamebase.Data() << cNORMAL
                << std::endl;
   }
   ...
   /*for (Int_t n = 0; n < SRIM_table.size(); n++){
       std::cout <<' '<< std::get<0> (SRIM_table.at(n))<<' '<<std::get<1> (SRIM_table.at(n))<<' '<<std::get<2>
   (SRIM_table.at(n))<<std::endl;
   ...
   */
   TF1 *enFit =
      new TF1("enFit", "[0]+[1]*x+[2]*pow(x,2)+[3]*pow(x,3)+[4]*pow(x,4)+[5]*pow(x,5)+[6]*pow(x,6)", 0, 200000);
   ...
   while (energy > 0) {
      x += step;
      inter = param0 + param1 * energy + param2 * pow(energy, 2) + param3 * pow(energy, 3) + param4 * pow(energy, 4) +
              param5 * pow(energy, 5) + param6 * pow(energy, 6);
      loss = inter * step;
      energy -= loss;
