...
void run_plot(TString FileNameHead = "output_proto_ana_10Be")
{
   ...
   TH2D *Range_hist[4];
   for (Int_t i = 0; i < 4; i++)
      Range_hist[i] = new TH2D(Form("Range[%i]", i), Form("Range[%i]", i), 100, 0, 180, 100, 0, 1000);

   TH1D *HoughSlope[4];
   for (Int_t i = 0; i < 4; i++)
      HoughSlope[i] = new TH1D(Form("Slope[%i]", i), Form("Slope[%i]", i), 100, -10, 10);

   TH1D *HoughInt[4];
   for (Int_t i = 0; i < 4; i++)
      HoughInt[i] = new TH1D(Form("Intercept[%i]", i), Form("Intercept[%i]", i), 750, 0, 750);
   ...
   if (!kineStrIn->fail()) {
      while (!kineStrIn->eof()) {
         *kineStrIn >> ThetaCMSIn[numKin] >> ThetaLabRecIn[numKin] >> EnerLabRecIn[numKin] >> ThetaLabScaIn[numKin] >>
            EnerLabScaIn[numKin];
         numKin++;
      }
   } else if (kineStrIn->fail())
      std::cout << " Warning : No Kinematics file found for this reaction! Please run the macro on "
                   "$SIMPATH/macro/Kinematics/Decay_kinematics/Mainrel.cxx"
                << std::endl;
   ...
   */
   ...
   while (Reader1.Next() && Reader2.Next()) {
      ...
      /*    if((backanglecut->IsInside(AngleFit->at(0),AngleFit->at(2)))||(backanglecut->IsInside(AngleFit->at(1),AngleFit->at(3))))
         { Q02_Kine->Fill(AngleFit->at(0),AngleFit->at(2)); Q02_Kine->Fill(AngleFit->at(1),AngleFit->at(3));
