...
void run_ana_8He(TString FileNameHead = "output_proto_reco", Int_t num_ev = 3000000, Int_t file_ini = 0,
                 Int_t file_end = 0, Int_t runnum = 250, Float_t HoughDist = 2.0, Bool_t debug = kTRUE,
                 Bool_t stdhough = kFALSE, TString file = "../Kinematics/Decay_kinematics/Kine.txt")
{

   // gStyle->SetCanvasPreferGL(1);
   // gStyle->SetPalette(1);
   if (!debug)
      gErrorIgnoreLevel = kFatal; // Avoid printing Minuit errors
   ...
   TH1I *DistHist[4];
   for (Int_t i = 0; i < 4; i++)
      DistHist[i] = new TH1I(Form("DistHist[%i]", i), Form("DistHit%i", i), 100, 0, 1000);
   ...
   for (Int_t i = 0; i < 4; i++) {
      ...
      HoughFitSTD[i] =
         new TF1(Form("HoughFitSTD%i", i), " (  (-TMath::Cos([0])/TMath::Sin([0]))*x ) + [1]/TMath::Sin([0])", 0, 120);
   ...
   for (Int_t i = 0; i <= file_end; i++) {
      if (i < 10)
         FileNameHead_num.Form("%i_000%i", runnum, i);
      else if (i < 100)
         FileNameHead_num.Form("%i_00%i", runnum, i);
      else if (i < 1000)
         FileNameHead_num.Form("%i_0%i", runnum, i);
      ...
      while (Reader1.Next() && nEve < num_ev) {
         ...
         // std::cout<<"
         // ==================================================================================="<<std::endl;
         // if(nEve%100==0) std::cout<<" Event number : "<<protoevent->GetEventID()<<" - Number of Hits :
         // "<<nHits<<std::endl; if(nEve%100==0)
         ...
         std::vector<std::pair<Double_t, Double_t>> HoughPar =
            fHoughSpaceLine_buff->GetHoughPar(); // No argument retrieve parameters from Hough Histogram
         std::vector<std::pair<Double_t, Double_t>> HoughParSTD;
         if (stdhough)
            HoughParSTD = fHoughSpaceLine_buff->GetHoughPar("STD");
         ...
         for (Int_t i = 0; i < 4; i++) {
            ...
            for (Int_t j = 0; j < qNumHit; j++) {
               ...
               Double_t geo_dist = TMath::Abs(TMath::Cos(HoughPar.at(i).first) * radius +
                                              TMath::Sin(HoughPar.at(i).first) * position.Z() - HoughPar.at(i).second);
               if (stdhough)
                  geo_dist_STD =
                     TMath::Abs(TMath::Cos(HoughParSTD.at(i).first) * radius +
                                TMath::Sin(HoughParSTD.at(i).first) * position.Z() - HoughParSTD.at(i).second);
        ...
        latex->SetTextColor(kRed);*/
               ...
               /*if(debug){
                      std::cout<<"
               ___________________________________________________________________________________________________"<<std::endl;
                       std::cout<<" Hit : "<<j<<" - Radius : "<<radius<<" - Z pos : "<<position.Z()<<"        - Distance
               to Hough Space : "<<mda<<" Geomtrical Distance : "<<geo_dist<<std::endl;
                        //std::cout<<" Pixel X : "<<pixelX<<" Pixel Y : "<<pixelY<<std::endl;
                        //std::cout<<" Pixel Back X : "<<pixelXback<<" Pixel Back Y : "<<pixelYback<<std::endl;

               }*/

               if (geo_dist < HoughDist)
                  HitPatternFilter[i]->SetPoint(HitPatternFilter[i]->GetN(), radius, position.Z());
               if (stdhough)
                  if (geo_dist_STD < HoughDist)
                     HitPatternFilterSTD[i]->SetPoint(HitPatternFilterSTD[i]->GetN(), radius, position.Z());
            ...
            if (debug)
               HitPattern[i] = new TGraph(qNumHit, rad_graph, posz_graph);
            if (debug && stdhough)
               HitPatternSTD[i] = new TGraph(qNumHit, rad_graph, posz_graph);

            if (debug)
               std::cout << cYELLOW << " Fitting Quadrant : " << i << cNORMAL << std::endl;
            ...
            if (HitPatternFilter[i]->GetN() > 3) {
               ...
               if (fitResultSTD[i] && stdhough) {
                  ...
                  if (par1STD >= 0)
                     afitSTD = TMath::ATan2(1, TMath::Abs(par1));
                  else if (par1STD < 0)
                     afitSTD = TMath::Pi() - TMath::ATan2(1, TMath::Abs(par1));
               }
            }
         ...
         if ((qRad.at(0) > 5 && qRad.at(2) > 5) && (qNumHits.at(0) > 5 && qNumHits.at(2) > 5) &&
             TMath::Abs(par0_fit.at(0) - par0_fit.at(2)) < 50) {
            ...
            if (debug) {
               ...
               std::cout << " Angle from Hough Space Quadrant 0 : " << Angle.at(0)
                         << " Angle from Hough Space Quadrant 2 : " << Angle.at(2) << std::endl;
               std::cout << " Angle from Fit Quadrant 0 : " << Angle_fit.at(0)
                         << " Angle from Fit Quadrant 2 : " << Angle_fit.at(2) << std::endl;
            }
         }
         if ((qRad.at(1) > 5 && qRad.at(3) > 5) && (qNumHits.at(1) > 5 && qNumHits.at(3) > 5) &&
             TMath::Abs(par0_fit.at(1) - par0_fit.at(3)) < 50) {
            ...
            if (debug) {
               ...
               std::cout << " Angle from Hough Space Quadrant 1 : " << Angle.at(1)
                         << " Angle from Hough Space Quadrant 3 : " << Angle.at(3) << std::endl;
               std::cout << " Angle from Fit Quadrant 1 : " << Angle_fit.at(1)
                         << " Angle from Fit Quadrant 3 : " << Angle_fit.at(3) << std::endl;
            }
         }

         if (stdhough) {
            if ((qRad.at(0) > 5 && qRad.at(2) > 5) && (qNumHits.at(0) > 5 && qNumHits.at(2) > 5) &&
                TMath::Abs(par0_fitSTD.at(0) - par0_fitSTD.at(2)) < 50) {
               Q13_Kine->Fill(Angle_fitSTD.at(0), Angle_fitSTD.at(2));
               Q13_Kine_buff->Fill(Angle_fitSTD.at(0), Angle_fitSTD.at(2));
            }
            if ((qRad.at(1) > 5 && qRad.at(3) > 5) && (qNumHits.at(1) > 5 && qNumHits.at(3) > 5) &&
                TMath::Abs(par0_fitSTD.at(1) - par0_fitSTD.at(3)) < 50) {
         ...
         if (debug) {
            ...
            if (stdhough)
               HoughFitSTD[0]->Draw("SAME");

            c1->cd(2);
            HitPattern[1]->Draw("A*");
            HoughFit[1]->Draw("SAME");
            if (stdhough)
               HoughFitSTD[1]->Draw("SAME");

            c1->cd(3);
            HitPattern[2]->Draw("A*");
            HoughFit[2]->Draw("SAME");
            if (stdhough)
               HoughFitSTD[2]->Draw("SAME");

            c1->cd(4);
            HitPattern[3]->Draw("A*");
            HoughFit[3]->Draw("SAME");
            if (stdhough)
               HoughFitSTD[3]->Draw("SAME");
