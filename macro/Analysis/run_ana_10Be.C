...
void run_ana_10Be(Int_t num_ev = 6000, Int_t file_ini = 0, Int_t file_end = 0, Int_t runnum = 571,
                  Float_t HoughDist = 2.0, Bool_t debug = kFALSE, Bool_t stdhough = kFALSE,
                  TString file = "../Kinematics/Decay_kinematics/Kine.txt")
{
   ...
   for (Int_t i = 0; i <= file_end; i++) {
      ...
      while (Reader1.Next() && nEve < num_ev) {
         ...
         // ==================================================================================="<<std::endl;
         if (nEve % 100 == 0)
            std::cout << " Event number : " << event->GetEventID() << " - Number of Hits : " << nHits << std::endl;
         if (debug)
        ...
        latex->SetTextColor(kRed);*/
            ...
            if (debug)
               std::cout << " Fitting Quadrant : " << i << std::endl;
            if (qNumHit > 3) {
         ...
         if (qFitStatus[0] + qFitStatus[1] + qFitStatus[2] + qFitStatus[3] == 4)
            goodFit = kTRUE;
         // cout<<goodFit<<endl;
         ...
         if ((qRad.at(0) > 5 && qRad.at(2) > 5) && (qNumHits.at(0) > 5 && qNumHits.at(2) > 5) &&
             (TMath::Abs(par0_fit.at(0) - par0_fit.at(2)) < 50) && goodFit) {
         ...
         if ((qRad.at(1) > 5 && qRad.at(3) > 5) && (qNumHits.at(1) > 5 && qNumHits.at(3) > 5) &&
             (TMath::Abs(par0_fit.at(1) - par0_fit.at(3)) < 50) && goodFit) {
