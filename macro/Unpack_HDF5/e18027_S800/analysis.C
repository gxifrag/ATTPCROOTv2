...
void analysis(int runNumberS800, int runNumberATTPC)
{
   ...
   TString digiFileName =
      TString::Format("/mnt/analysis/e18027/rootMerg/run_%04d_%04d.root", runNumberS800, runNumberATTPC);
   ...
   TString outFileNameHead =
      TString::Format("/mnt/analysis/e18027/rootAna/runAnalyzed_%04d_%04d.root", runNumberS800, runNumberATTPC);
   ...
   for (Int_t i = 0; i < nEvents; i++) {
      ...
      // if(trackCand.size()>0) std::cout<<i<<" "<<s800cal->GetTS()<<"   "<<s800cal->GetSCINT(0)->GetDE()<<"
      // "<<trackCand.size()<<"  "<<s800cal->GetIsInCut()<<" "<<s800cal->GetCRDC(0)->GetX()<<std::endl;
      ...
      */
      ...
      if (trackCand.size() > 0 && s800cal->GetIsInCut() == kTRUE) {
         ...
         for (int k = 0; k < S800_timeMTDCXf.size(); k++) {
            ...
            if (S800_timeMTDCXf.at(k) > 170 && S800_timeMTDCXf.at(k) < 230)
         ...
         for (int k = 0; k < S800_timeMTDCObj.size(); k++) {
            ...
            if (S800_timeMTDCObj.at(k) > -75 && S800_timeMTDCObj.at(k) < 0)
         ...
         mom_proton1_reco.SetZ (mom1_norm_reco * TMath::Cos(theta1));
            ...
            std::cout << "Theta=" << th_alpha_lab << "\t" << range_p1 / 10.0 << "\t" << eLoss_p1_reco << "\t" << MaxR1
                      << std::endl;
