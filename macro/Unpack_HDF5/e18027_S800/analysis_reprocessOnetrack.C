...
// If plots make no sens first check that the referential of the experimental setup is really what we use in simulation
// (z axis direction) Inverse map calculations will change for now it is not suited for a 1m target...
...
void analysis_reprocessOnetrack(int runNumberS800, int runNumberATTPC)
{
   ...
   TString digiFileName =
      TString::Format("/mnt/analysis/e18027/rootAna/reprocess/reprocess_%04d_%04d.root", runNumberS800, runNumberATTPC);
   ...
   */
   ...
   TString outFileNameHead =
      TString::Format("/mnt/analysis/e18027/rootAna/anaRepro_%04d_%04d.root", runNumberS800, runNumberATTPC);
   ...
   for (Int_t i = 0; i < nEvents; i++) {
      ...
      */
         ...
         for (Int_t w = 0; w < trackCand.size(); w++) {
            ...
            range_p1 = 0.;
            eLoss_p1_reco = 0.;
            mom1_norm_reco = 0.; // reset variables
            theta_cm = 0.;
            Ex4 = 0.;
            MaxR1 = 0.;
            MaxZ1 = 0.;
            ...
            // no stop in chamber or in the beam hole if(MaxR1>=242. || MaxR2>=242.  || MaxZ1>973. || MaxZ2>973.)
         ...
         mom_proton1_reco.SetZ (mom1_norm_reco * TMath::Cos(theta1));
            ...
            // if((theta1*180/3.1415>20 && theta1*180/3.1415<80) || (theta1*180/3.1415>100 && theta1*180/3.1415<160) &&
            // (range_p1<230))theta_r_he2_reco->Fill(theta1*180/3.1415);
