...
void analysis_reprocess()
{
   ...
   */
   ...
   for (Int_t i = 0; i < nEvents; i++) {
      ...
      // std::cout<<s800cal->GetTS()<<"   "<<s800cal->GetSCINT(0)->GetDE()<<"  "<<trackCand.size()<<"
      // "<<s800cal->GetIsInCut()<<std::endl;
      ...
      s800cal->GetHODOSCOPE(j)->GetEnergy()<=4000) S800_hodoSum +=
      s800cal->GetHODOSCOPE(j)->GetEnergy()*3000./coeff_hodo[j]; std::vector <double> S800_invMapOut =
      get_invmap_vars(inv_map,S800_x0,S800_y0,S800_afp,S800_bfp); S800_ata = S800_invMapOut.at(0); S800_bta =
      S800_invMapOut.at(1); S800_yta = S800_invMapOut.at(2); S800_dta = S800_invMapOut.at(3); S800_thetaLab=
      S800_invMapOut.at(4); S800_phi= S800_invMapOut.at(5);
      ...
      */
         ...
         */
         ...
         for (Int_t w = 0; w < two_p_pair.size(); w++) {
            ...
            hBr0 =
               new TH1D(Form("Bragg_%d_evt_%d", tracknumb1, i), Form("Bragg_%d_evt_%d", tracknumb1, i), 120, 0, 600);
            Double_t Tcharge1 = 0;
            ...
            // std::cout << "Init   Hitarray size    "<<nHits << '\n';
            hBr1 =
               new TH1D(Form("Bragg_%d_evt_%d", tracknumb2, i), Form("Bragg_%d_evt_%d", tracknumb2, i), 120, 0, 600);
            Double_t Tcharge2 = 0;
            ...
            if (MaxR1 >= 242. || MaxR2 >= 242. || MaxZ1 > 973. || MaxZ2 > 973.)
               continue; // if do no stop in chamber or in the beam hole
