...
void PIDS800Corr(int runNumberS800)
{
   ...
   for (Int_t i = 0; i < nEvents; i++) {
      ...
      for (int k = 0; k < S800_timeMTDCXf.size(); k++) {
         if (S800_timeMTDCXf.at(k) > 140 && S800_timeMTDCXf.at(k) < 205)
            S800_timeXfSelect = S800_timeMTDCXf.at(k); // 150-315
      }
      for (int k = 0; k < S800_timeMTDCObj.size(); k++) {
         if (S800_timeMTDCObj.at(k) > -120 && S800_timeMTDCObj.at(k) < 50)
            S800_timeObjSelect = S800_timeMTDCObj.at(k); //-60-30
      }
      ...
      if (std::isnan(ObjCorr) == 1 || std::isnan(XfObj_tof) == 1 || std::isnan(S800_ICSum) == 1)
         continue;

      // if(CondMTDCXfObj)XfpObj_Obj->Fill(S800_timeObjSelect,XfObj_tof);
      if (CondMTDCXfObj)
         XfpObj_Obj->Fill(ObjCorr, XfObj_tof);
      // if(CondMTDCObj)ICSum_Obj->Fill(S800_timeObjSelect,S800_ICSum);
      if (CondMTDCObj)
         ICSum_Obj->Fill(ObjCorr, S800_ICSum);
