...
void PIDS800(int runNumberS800)
{
   ...
   for (Int_t i = 0; i < nEvents; i++) {
      ...
      for (int k = 0; k < S800_timeMTDCXf.size(); k++) {
         if (S800_timeMTDCXf.at(k) > 150 && S800_timeMTDCXf.at(k) < 315)
      ...
      if (std::isnan(S800_tofCorr) == 1 || std::isnan(S800_dECorr) == 1)
         continue;

      if (CondMTDCXfObj)
         XfpObj_Obj->Fill(S800_timeObjSelect, XfObj_tof);
      if (CondMTDCObj)
         ICSum_Obj->Fill(S800_timeObjSelect, S800_ICSum);
