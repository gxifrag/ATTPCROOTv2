...
void inv_map_inter()
{
   ...
   std::cout << "map test : " << fMap_v.at(0).at(0).at(0).coefficient << " " << fMap_v.at(0).at(1).at(1).coefficient
             << std::endl;
   std::vector<InvMapRow> invrow_v;
   std::vector<std::vector<double>> coeff_v;
   Int_t Gsize = 0, icoeff = 0;
   for (int i = 0; i < fMap_v.at(0).size(); i++)
      for (int j = 0; j < fMap_v.at(0).at(i).size(); j++)
         Gsize++;
      ...
      for (int j = 0; j < fMap_v.at(0).size();
         ...
         for (int k = 0; k < fMap_v.at(0).at(j).size(); k++) { // loop on coeff in par
            ...
            for (int i = 0; i < fMap_v.size(); i++) { // loop on maps
               std::cout << "mapcoeff : " << i << " " << j << " " << k << " " << fMap_v.at(i).at(j).at(k).coefficient
                         << std::endl;
               buff_v.push_back((Double_t)fMap_v.at(i).at(j).at(k).coefficient);
               // std::cout<<"invrow : "<<j<<" "<<invrow_v.at(j).coefficient<<std::endl;
            }
            switch (j) {
            case 0: outfile->cd("ATA"); break;
            case 1: outfile->cd("YTA"); break;
            case 2: outfile->cd("BTA"); break;
            case 3: outfile->cd("DTA"); break;
            }
            ...
            fGraph[icoeff]->Write();
            std::cout << "debug  : " << j << " " << k << " " << fMap_v.size() << " " << mapdist_v.back() << " "
                      << fSpline[icoeff]->Eval(0.35) << std::endl;
   ...
   std::cout << MapCalc_s(order, 0, input, 0.11) << " " << MapCalc_s(order, 0, input, 0.23) << " "
             << MapCalc_s(order, 0, input, 0.98) << std::endl;
