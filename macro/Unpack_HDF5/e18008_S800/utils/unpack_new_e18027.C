...
void unpack_new(int runNumberS800, int runNumberATTPC)
{
   ...
   // TString inputFile =
   // TString::Format("/mnt/simulations/ceclub/giraud/attpc/ATTPCROOTv2/macro/Unpack_HDF5/hdf5Files/run_0002.h5",
   // runNumberATTPC);

   // Set the output file
   //  TString outputFile = TString::Format("/mnt/analysis/e18027/rootMerg/run_2%03d_%04d.root", runNumberS800,
   //  runNumberATTPC);
   // TString outputFile = TString::Format("/mnt/analysis/e18027/rootMerg/run_%04d_%04d.root", runNumberS800,
   // runNumberATTPC);
   TString outputFile = TString::Format("run_%04d_%04d.root", runNumberS800, runNumberATTPC);
   // TString outputFile =
   // TString::Format("/projects/ceclub/giraud/attpc/ATTPCROOTv2/macro/Simulation/d2He/Analyis_d2He/merged_run0002_48Ca.root",
   ...
   // TString S800File =
   // "/projects/ceclub/giraud/s800root_jorge/s800root/files/rootFiles/cal/test-runs800-48Ca-CAL-0001-new.root"; TString
   // S800File = TString::Format("/mnt/analysis/e18027/rootS800/cal/run-2%03d-00.root", runNumberS800);
   ...
   // MergeEvt->SetTsDelta(844107828);//run180.. high pressure, uncomment the lines with "special for run180" in
   // ATMergeTask MergeEvt->SetTsDelta(0);//run180.. high pressure
   ...
   */
   ...
   std::cout << " Check MergeTask :: S800 events " << YELLOW << MergeEvt->GetS800TsSize() << RESET << " AT-TPC events "
             << YELLOW << numEvents << RESET << "  Merged TS size " << YELLOW << MergeEvt->GetMergedTsSize() << RESET
             << std::endl;
   if ((double)MergeEvt->GetMergedTsSize() / (double)MergeEvt->GetS800TsSize() < 1)
      std::cout << " ! WARNING Ratio (merged/S800) " << RED
                << (double)MergeEvt->GetMergedTsSize() / (double)MergeEvt->GetS800TsSize() << RESET << std::endl;
   if ((double)MergeEvt->GetMergedTsSize() / (double)MergeEvt->GetS800TsSize() == 1)
      std::cout << " Ratio (merged/S800) " << GREEN
                << (double)MergeEvt->GetMergedTsSize() / (double)MergeEvt->GetS800TsSize() << RESET << std::endl;
   std::cout << " Check MergeTask :: S800 events " << MergeEvt->GetS800TsSize() << " AT-TPC events " << numEvents
             << "  Merged TS size " << MergeEvt->GetMergedTsSize() << std::endl;
   std::cout << std::endl;
