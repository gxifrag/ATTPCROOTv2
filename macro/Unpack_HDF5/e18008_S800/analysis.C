...
static double coeff_hodo[32] = {100000000, 1164, 887, 100000000, 824, 1728, 826, 100000000, 941,  962, 875,
                                915,       875,  835, 1079,      842, 855,  870, 891,       1786, 960, 940,
                                825,       625,  990, 2195,      805, 870,  575, 1004,      1164, 748};
...
double eloss_approx(double rangep)
{ // rangep in mm, elossp in MeV; get this approximated function by fitting the GEANT4 E vs R obtained with the
  // simulation
   double elossp;
   if (rangep < 131.)
      elossp = 0.0833284 + 0.0118757 * rangep - 7.3698e-05 * pow(rangep, 2) + 2.42758e-07 * pow(rangep, 3);
   if (rangep >= 131.)
      elossp = 0.36519 + 0.004761 * rangep - 4.28371e-06 * pow(rangep, 2) + 2.35727e-09 * pow(rangep, 3);
   return elossp;
}
...
void analysis(int runNumberS800, int runNumberATTPC)
{
   ...
   TH2D *thetacm_epsilon_pp_reco =
      new TH2D("thetacm_epsilon_pp_reco", "#theta_{cm} #epsilon_{pp} #^{2}He", 200, 0, 20, 100, 0, 10);
   //----- S800
   ...
   Double_t lastX1 = 0., lastX2 = 0., lastY1 = 0., lastY2 = 0., lastZ1 = 0., lastZ2 = 0., vertexX = 0., vertexY = 0.,
            vertexZ = 0.;
   ...
   Double_t S800_timeRf = 0., S800_x0 = 0., S800_x1 = 0., S800_y0 = 0., S800_y1 = 0., S800_E1up = 0., S800_E1down = 0.,
            S800_tof = 0., S800_tofCorr = 0., S800_dE = 0., S800_dECorr = 0., S800_hodoSum = 0., S800_afp = 0.,
            S800_bfp = 0., S800_ata = 0., S800_bta = 0., S800_yta = 0., S800_dta = 0., S800_thetaLab = 0.,
            S800_phi = 0., S800_timeE1up = 0., S800_timeE1down = 0., S800_timeE1 = 0., S800_timeXf = 0.,
            S800_timeObj = 0.;
   ...
   for (Int_t i = 0; i < nEvents; i++) {
      ...
      if (trackCand.size() > 1 && s800cal->GetIsInCut() == kTRUE) {
         ...
         for (int k = 0; k < S800_timeMTDCXf.size(); k++) {
            if (S800_timeMTDCXf.at(k) > 160 && S800_timeMTDCXf.at(k) < 240)
               S800_timeXfSelect = S800_timeMTDCXf.at(k);
         }
         for (int k = 0; k < S800_timeMTDCObj.size(); k++) {
            if (S800_timeMTDCObj.at(k) > -120 && S800_timeMTDCObj.at(k) < -20)
         ...
         for (Int_t j = 0; j < 32; j++)
            if (s800cal->GetHODOSCOPE(j)->GetEnergy() >= 10 && s800cal->GetHODOSCOPE(j)->GetEnergy() <= 4000)
               S800_hodoSum += s800cal->GetHODOSCOPE(j)->GetEnergy() * 3000. / coeff_hodo[j];
         ...
         for (Int_t w = 0; w < trackCand.size() - 1; w++) {
            ...
            for (Int_t z = w + 1; z < trackCand.size(); z++) {
               ...
               if (vertex0 == vertex1)
                  two_p_pair.push_back(std::make_pair(w, z));
            }
         }
         ...
         */
         ...
         for (Int_t w = 0; w < two_p_pair.size(); w++) {
            ...
            MaxR2 = 0.;
            MaxZ1 = 0.;
            MaxZ2 = 0., beam_theta = 0., beam_phi = 0.;
            ...
            std::vector<double> S800_invMapOut = get_invmap_vars(inv_map, S800_x0 / 1000., S800_y0 / 1000., S800_afp,
                                                                 S800_bfp, 0.8 - vertexMean.Z() / 1.0e3);
            S800_ata = S800_invMapOut.at(0);
            S800_bta = S800_invMapOut.at(1);
            S800_yta = S800_invMapOut.at(2);
            S800_dta = S800_invMapOut.at(3);
            ...
            /*
                    ...
                    //std::cout << "Init   Hitarray size    "<<nHits << '\n';
                     Int_t Nbins=120;
                     hBr0 = new
               TH1D(Form("Bragg_%d_evt_%d",tracknumb1,i),Form("Bragg_%d_evt_%d",tracknumb1,i),Nbins,0,600) ; Double_t
               Tcharge1 = 0; vector<Double_t> vRange1; vector<Double_t> vCharge1;
                  ...
                  TF1 *RCFunc1 = new TF1("RCFunc1",aGraph1toFunc,0,vRange1.back());
                        ...
                        //std::cout << "Init   Hitarray size    "<<nHits << '\n';
                        hBr1 = new
               TH1D(Form("Bragg_%d_evt_%d",tracknumb2,i),Form("Bragg_%d_evt_%d",tracknumb2,i),Nbins,0,600) ; Double_t
               Tcharge2 = 0; vector<Double_t> vRange2; vector<Double_t> vCharge2;
      ...
      */
            ...
            // if(MaxR1>=242. || MaxR2>=242. || MaxR1<=25. || MaxR2<=25. || MaxZ1>973. || MaxZ2>973.) continue; //if do
            // no stop in chamber or in the beam hole
            if (MaxR1 >= 245. || MaxR2 >= 245. || MaxZ1 > 1000. || MaxZ2 > 1000.)
               continue;
            ...
            /*
               ...
               lastPoint1).first;//GetThetaPhi(..,..,1) for data theta2 =
               trackCand.at(two_p_pair[w].second).GetThetaPhi(vertexMean, lastPoint2).first;//GetThetaPhi(..,..,1) for
               data phi1 = trackCand.at(two_p_pair[w].first).GetThetaPhi(vertexMean,
               lastPoint1).second;//GetThetaPhi(..,..,1) for data phi2 =
               trackCand.at(two_p_pair[w].second).GetThetaPhi(vertexMean, lastPoint2).second;//GetThetaPhi(..,..,1) for
            */
            theta1 = trackCand.at(two_p_pair[w].first)
                        .GetThetaPhi(vertexMean, lastPoint1, 1)
                        .first; // GetThetaPhi(..,..,1) for data
            theta2 = trackCand.at(two_p_pair[w].second)
                        .GetThetaPhi(vertexMean, lastPoint2, 1)
                        .first; // GetThetaPhi(..,..,1) for data
            phi1 = trackCand.at(two_p_pair[w].first)
                      .GetThetaPhi(vertexMean, lastPoint1, 1)
                      .second; // GetThetaPhi(..,..,1) for data
            phi2 = trackCand.at(two_p_pair[w].second)
                      .GetThetaPhi(vertexMean, lastPoint2, 1)
                      .second; // GetThetaPhi(..,..,1) for data
            ...
            TVector3 vp1(TMath::Sign(1, lastX1) * fabs(fitPar1[1]), TMath::Sign(1, lastY1) * fabs(fitPar1[3]),
                         TMath::Sign(1, (lastZ1 - vertexZ)) * fabs(fitPar1[5]));
            TVector3 vp2(TMath::Sign(1, lastX2) * fabs(fitPar2[1]), TMath::Sign(1, lastY2) * fabs(fitPar2[3]),
