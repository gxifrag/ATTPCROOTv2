            ...
            if (track.GetHitArray()->size() > 10 && track.GetIsNoise() == 0 && tracks.size() <= 2) {
                  ...
                  if (TMath::Sqrt(pos.X() * pos.X() + pos.Y() * pos.Y()) > 110) {
                     ...
                     for (int indTB = 0; indTB < 512; ++indTB) {
                        if (adc[indTB] > 40.0) {
                           ...
                           if (Qtot_tree > 180000) {
                              outputFileBragg << "	" << indTB << "		" << adc[indTB] << "	" << Qtot_tree << "		"
                                              << i << "		" << angDegOut << "\n";
                           }
                        }
                     }
               ...
               if (isValid) {
                  ...
                  if (angDeg < 80.0 && angDeg > 10.0) {
                     ...
                     double dummy_result = chi2fit(mesh, Qprot_ref, Qlith_ref, Qtot, angDeg, firstTBOT, chi2min,
                     ...
                     /*for(int indTB=0;indTB<512;++indTB)
                  {
                    outputFile<<indTB<<"	"<<mesh->GetBinContent(indTB)<<"	"<<mesh->GetBinError(indTB)<<"	"<<i<<"
                  "<<angDeg<<"\n";
