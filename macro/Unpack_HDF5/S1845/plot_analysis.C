Double_t ConvolutedDecay(Double_t *x, Double_t *par)
{
   ...
   /// This function is derived from the convolution of a gaussian with an exponential decay, to fit TAC spectra of long
   /// half-lives (above 100 ps) Requires the following parameters:
   ...
   Double_t val;
   val = TMath::Sqrt(TMath::Pi()) * par[0] * par[3] / 2 *
            TMath::Exp(par[3] / 2 * (2 * par[1] + par[3] * pow(par[2], 2) - 2 * x[0])) *
            TMath::Erfc((par[1] + par[3] * pow(par[2], 2) - x[0]) / (TMath::Sqrt(2) * par[2])) +
         par[4];
   return val;
}
...
void plot_analysis()
{
   ...
   std::vector<TString> files{
      ...
      "run_0102_analysis.root", "run_0105_analysis.root", "run_0106_analysis.root", "run_0107_analysis.root",
      "run_0108_analysis.root", "run_0110_analysis.root", "run_0111_analysis.root", "run_0112_analysis.root",
      "run_0113_analysis.root", "run_0114_analysis.root", "run_0115_analysis.root", "run_0116_analysis.root",
      "run_0118_analysis.root", "run_0119_analysis.root", "run_0142_analysis.root", "run_0144_analysis.root",
      "run_0145_analysis.root", "run_0149_analysis.root", "run_0150_analysis.root", "run_0153_analysis.root",
      "run_0154_analysis.root", "run_0156_analysis.root", "run_0157_analysis.root", "run_0158_analysis.root"};
   ...
   for (auto ifile : files) {
      ...
      for (auto iEvent = 0; iEvent < nEvents; ++iEvent) {
         ...
         if (chi2_tree > 0 && alphaTrigger_tree < 200) {
            ...
            // std::cout<<" Event "<<eventNum_tree<<" chi2 proton "<<chi2_tree<<" - chi2 7Li "<<chi2Li_tree<<" - stretch
            // proton "<<stretch_tree<<" - stretch 7Li "<<stretchLi_tree<<"\n";
            ...
            if (stretch_tree > -0.25 && stretch_tree < 1.5 && stretchLi_tree > 0.0 && stretchLi_tree < 1.5 &&
                angle_tree > 30 && angle_tree < 70 && shift_tree < -10 && shift_tree > -45 && shiftLi_tree < 0 &&
                shiftLi_tree > -20 && Qexp_tree > 50000 && Qexp_tree < 300000 && chi2_tree > 0 && chi2_tree < 200 &&
                chi2Li_tree > 1000
