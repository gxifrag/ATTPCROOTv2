...
Double_t chi2fitImp(int ioptionpart, TH1F *mesh, double &sumref, double sumexp, double angle, int iTb, double &chimin,
                    double &shiftmin, double &stretchmin, std::vector<double> &ref_curve,
                    std::vector<double> &exp_curve)
{
   ...
   for (int iter = 0; iter < 1; ++iter) {
      for (int istretch = 0; istretch < istretchmax; ++istretch) {
         ...
         for (int itb = 200; itb < 400; ++itb) {
            ...
            double fitfit = 0;
            if (ioptionpart == 1)
               fitfit = fitref(tbref); //! function fitted at point bref corrected for projection on exp axis
            if (ioptionpart == 2)
               fitfit = fitLi(tbref);
...
Double_t chi2fit(TH1F *mesh, double Qrefproton, double Qreflith, double Qrefexp, double angle, int iTb, double &chimin,
                 double &shiftmin, double &stretchmin, std::vector<double> &ref_curve, std::vector<double> &exp_curve)
{
      ...
      for (int istretch = 0; istretch < istretchmax; ++istretch) {
         ...
         for (int izero = 0; izero < izeromax; ++izero) {
            ...
            for (int itb = 100; itb < 400; ++itb) {
               ...
               int tbref =
                  itb + shift +
                  stretch * (itb - itb0); //! this is the coordinate of the analytical function before angle correction
               ...
               double fitfit =
                  fitref(tbref) /
                  cos(angle * 0.01745); // !function fitted at point bref corrected for projection on exp axis

               double sigma = (mesh->GetBinContent(itb) + fitfit) * 0.1 + 100.; //  !error to be tested
               sigma = 100.;                                                    //  !error to be tested
               chi2[istretch][izero] =
                  chi2[istretch][izero] +
                  TMath::Power(((mesh->GetBinContent(itb) * Qrefproton / Qrefexp - fitfit) / sigma),
                               2); // !now with renormalized
   ...
   for (Int_t i = 0; i < nEvents; i++) {
      ...
      if (event != NULL) {
         ...
         std::cout << " Number of pads : " << padArray->size()
                   << " - Number of auxiliary pads : " << auxPadArray->size() << "\n";
         ...
         for (auto track : tracks) {
            // std::cout<<" - Number of hits : "<<track.GetHitArray()->size()<<"	- Is noise? :
            // "<<track.GetIsNoise()<<"\n";
            ...
            if (track.GetHitArray()->size() > 10 && track.GetIsNoise() == 0 && tracks.size() <= 2) {
               ...
               for (auto hit : *hits) {
                  ...
                  if (TMath::Sqrt(pos.X() * pos.X() + pos.Y() * pos.Y()) > 110) {
                     ...
                     double distFromFirst = TMath::Sqrt(TMath::Power(pos.X() - first_pos.X(), 2) +
                                                        TMath::Power(pos.Y() - first_pos.Y(), 2) +
                                                        TMath::Power(pos.Z() - first_pos.Z(), 2));
                     double distFromLast =
                        TMath::Sqrt(TMath::Power(pos.X() - last_pos.X(), 2) + TMath::Power(pos.Y() - last_pos.Y(), 2) +
                                    TMath::Power(pos.Z() - last_pos.Z(), 2));
               ...
               if (isValid) {
                  ...
                  if (angDeg < 80.0 && angDeg > 10.0) {
                     ...
                     range_vs_Q->Fill(
                        track.GetLinearRange(),
                        event
                           ->GetEventCharge()); // For the moment only events where the clustering works 1 track + noise
                     ...
                     double dummy_result = chi2fitImp(ioptionpart, mesh, sumrefp, sumexp, angDeg, firstTBOT, chi2min,
                                                      shiftmin, stretchmin, ref_curve_tree, exp_curve_tree);
                     ...
                     dummy_result = chi2fitImp(ioptionpart, mesh, sumrefLi, sumexp, angDeg, firstTBOT, chi2min,
                                               shiftmin, stretchmin, refLi_curve_tree, exp_curve_tree);
