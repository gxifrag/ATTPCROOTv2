void run_unpack_alphasource(TString runNumber = "0664", TString parameterFile = "ATTPC.e17504.par",
                            TString mappath = "/Users/csantama/ATTPCROOT/resources/")
{
   ...
   /*
    *     Unpacking options:
    *         - SetUseSeparatedData:      To be used with 40 CoBo files without merging. Mainly for the ATTPC. Enabled
   ...
   if (fUseSeparatedData)
      fDecoderTask->SetPseudoTopologyFrame(
         kTRUE); //! This calls the method 40 times so for less than 40 Asads ATCore2 must be modified
   // fDecoderTask -> SetPositivePolarity(kTRUE);
   fDecoderTask->SetPersistence(kFALSE);
   fDecoderTask->SetMap(scriptdir.Data());
   fDecoderTask->SetInhibitMaps(inimap, lowgmap,
                                xtalkmap); // TODO: Only implemented for fUseSeparatedData!!!!!!!!!!!!!!!!!!!1
   fDecoderTask->SetNumCobo(40);
   fDecoderTask->SetMapOpt(0); // ATTPC : 0  - Prototype: 1 |||| Default value = 0

   // #NOTE: Used if more than 1 .graw file per Asad AND in numerical order, which the current macro "list_filename.pl"
   // does not do!!
   if (!fUseSeparatedData)
      ...
      while (dataFileWithPath.ReadLine(listFile)) {
         ...
         char Ctry = pch[0];
         iAsad = (int)Ctry - '0';
         if (abs(iAsad - iAsad_Last) > 0.1)
            iThread++;
         // std::cout << "File = " << dataFileWithPath << std::endl;
         // std::cout << "AsAd = " << iAsad << " ; previous AsAd = " << iAsad_Last << " ; iThread = " << iThread <<
         // std::endl;
