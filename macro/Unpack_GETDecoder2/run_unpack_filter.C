...
void run_unpack_filter(TString dataFile = "runfiles/ar46_run_0085.txt", TString parameterFile = "ATTPC.e15503b.par",
                       TString mappath = "/data/ar46/run_0085/")
{
   ...
   if (!fUseSeparatedData)
      ...
      while (dataFileWithPath.ReadLine(listFile)) {
         ...
         if (!check_file(dataFileWithPath.Data())) {
            std::cout << cRED << " GRAW file " << dataFileWithPath.Data() << " not found! Terminating..." << cNORMAL
                      << std::endl;
            exit(0);
         } else {
   ...
   psaTask->SetTimeCorrection(
      kFALSE); // NB: This method should be used carefully. Other calibrations coudl be different. Interpolation around
               // the maximum of the signal peak. Only affect Z calibration at PSA stage
   psaTask->SetMeanK(10);
