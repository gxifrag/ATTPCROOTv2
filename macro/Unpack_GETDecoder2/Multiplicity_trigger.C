...
void Multiplicity_trigger()
{
   ...
   // First, we transform what is written in the config file for the trigger threshold for each channel to a more useful
   // value.
   //*************************************************************************************************
   ...
   int tbIdx;
   int cobo;
   int CoboNumArray[10240];
   int PadNumArray[10241];
   double multiplicity[512];
   double Mutant_multiplicity;
   double MuTanT_multiplicity[1000];
   ...
   while (1) {
      ...
      if (!in.good())
         break;
      if (nlines < 5)
         printf("CoboNum=%8f, asad=%8f, aget=%8f, channel=%8f, pad=%8f\n", CoboNum, asad, aget, channel, pad);
   ...
   for (int i = 7; i < 8; i++) {
      ...
      for (Int_t iHit = 0; iHit < numHits; iHit++) {
         ...
         if (i == 0 && iHit < 100) {
            hRawPulse.push_back(
               new TH2I(Form("hRawPulse_Pad%i", iHit), Form("hRawPulse_Pad%i", iHit), 512, 0, 512, 500, 0, 4100));
         }

         // LOOP over the TIME distribution of each signal of the EVENT

         for (int j = 0; j < 512; j++) {
            if (MaxRawADC <= fRawAdc[j])
               MaxRawADC = fRawAdc[j];
            if (i == 0 && iHit < 100) {
               hRawPulse.back()->Fill(j, fRawAdc[j]);
            }
         }
         ...
         while (tbIdx < 512) {
            if (fRawAdc[tbIdx] > pad_threshold) {
               for (Int_t ii = tbIdx; ii < min(tbIdx + trigger_width, 512); ii++) {
                  trigSignal[ii] += trigger_height;
               }
               tbIdx += trigger_width;
         ...
         // sliding time window. The input of this part will be the triggerSignals calculated before, and the output
         // will be the multiplicity signals.
         //********************************************************************************************************************************
         ...
         for (Int_t kk = 0; kk < 10; kk++) {
            if (kk == cobo) {
               for (Int_t k = 0; k < 512; k++) {
                  if (MuTanT_multiplicity[k] > multiplicity_threshold) {
                     trigger = kTRUE;
                  }
               }
            }
   ...
   can1->Divide(2, 1);
   can1->cd(2);
   h_MuTant_Multiplicity->Draw("E PMC");
