...
void run_unpack_alpha_RANSAC_Ana(TString dataFile = "runfiles/NSCL/alphas/alpha_run_0100.txt",
                                 TString parameterFile = "ATTPC.alpha.par", TString mappath = "/data/ar46/run_0085/")
{
   ...
   if (!fUseSeparatedData)
      ...
      Int_t numLin = std::count(std::istreambuf_iterator<char>(listFile), std::istreambuf_iterator<char>(), '\n');
