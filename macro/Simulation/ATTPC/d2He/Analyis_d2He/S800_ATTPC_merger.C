...
double glom =
   2.; // in unit of clock ticks (glom of 1us is 10 units : 10*100ns), case by case not a constant for every experiment
int TsEvtS800Size = 0;
...
{
   bool is = false;
   if (ts1 > 0 && ts2 > 0 && fabs(ts1 - ts2) < glom)
      is = true;
   return is;
}

// fills the timestamps vectors for S800 and ATTPC
void TsEvt(TString str, const Int_t src, vector<Long64_t> &ts,
           vector<Double_t> &ievt) // src 0 || 1 : S800; src 2 || 3 : ATTPC
{
...
vector<int> EvtMatch(const vector<Long64_t> &TsEvtS800, const vector<Long64_t> &TsEvtATTPC, TF1 *fit)
{
   ...
   vector<int> match; // the index correspond to the S800 root entry number and the content is the ATTPC root entry
                      // number matching by TS
   for (int i = 0; i < TsEvtS800Size; i++) {
      if (i > 0 && isInGlom(TsEvtS800.at(i - 1), TsEvtS800.at(i)))
         cout << " -- Warning -- Timestamp of consecutive entries from S800 root file within the glom" << endl;
      else {
         match.push_back(-1);
         minj = fit->Eval(TsEvtS800.at(i)) - delj; // define the ATTPC entries range where the matching timestamp should
                                                   // be, to not loop over all the ATTPC entries.
         ...
         for (int j = minj; j < maxj; j++) {
            ...
            if (j > 0 && j < TsEvtATTPCSize && isInGlom(TsEvtATTPC.at(j - 1), TsEvtATTPC.at(j))) {
               ...
               match.at(i) = j;
               // cout<<" in glom "<<minj<<" "<<maxj<<" "<<i<<" "<<j<<" "<<TsEvtS800.at(i)<<" "<<TsEvtATTPC.at(j)<<"
               // "<<match.at(i)<<endl;
               continue;
            }
         }
...
void MergeTrees(TString strS800, TString strATTPC, const vector<int> &match, const Int_t S800class,
                const Int_t ATTPCclass)
{
   ...
   if (S800class == 0)
      readerValueS800Evt = new TTreeReaderValue<S800Event>(readerS800, "s800event");
   else if (S800class == 1)
      readerValueS800Calc = new TTreeReaderValue<S800Calc>(readerS800, "s800calc");
   ...
   if (S800class == 0)
      newTree->Branch("s800", &fS800);
   else if (S800class == 1)
      newTree->Branch("s800", &fS800Calc);

   ATEvent *eventATTPC0 = new ATEvent();
   ATRANSACN::ATRansac *eventATTPC1 = new ATRANSACN::ATRansac();
   if (ATTPCclass == 2)
      newTree->Branch("attpc", &eventATTPC0);
   else if (ATTPCclass == 3)
      newTree->Branch("attpc", &eventATTPC1);
   ...
   while (readerS800.Next()) {
      if (match.at(EvtS800) > 0) {
         if (S800class == 0) {
            ...
            *fS800Calc = (S800Calc)*readerValueS800Calc->Get();

         readerATTPC.SetEntry(match.at(EvtS800)); // event AT-TPC that matches with EvtS800

         if (ATTPCclass == 2)
            eventATTPC0 = (ATEvent *)readerValueATTPC->At(0);
         else if (ATTPCclass == 3)
            eventATTPC1 = dynamic_cast<ATRANSACN::ATRansac *>(readerValueATTPC->At(0));
...
int S800_ATTPC_merger()
{
   ...
   TString fileATTPC =
      "/home/juan/FairRoot/ATTPCROOTv2_simon/attpcdigi_d2He_1000.root"; // projects/ceclub/giraud/ATTPCROOTv2/macro/Unpack_HDF5/
   Int_t S800class, ATTPCclass;
   S800class = 1;  // 0 : rawtree; 1 : caltree
   ATTPCclass = 3; // 2 : raw hit pattern; 3: ransac fitted hit pattern

   TString FILEstr[4] = {
      fileS800_1, fileS800_2, fileATTPC,
      fileATTPC}; // maybe too complicated for nothing, could be better to define const strings at the beginning
