...
double *kine_2b(double m1, double m2, double m3, double m4, double K_proj, double thetalab, double K_eject)
{
   ...
   J_LtoCM = abs(((omega(s, pow(m1, 2), pow(m2, 2)) * omega(s, pow(m3, 2), pow(m4, 2))) / (4 * s * p1 * p3)) *
...
std:
   ...
   // for(Int_t iEvent=0; iEvent<nEvents; iEvent++)
   {
         ...
         if (iEvent % 2 != 0 && trackID != 1 && VolName == "drift_volume") { // solo los protones
            ...
            */
            ...
            if (trackID == 2) {
               ...
               if (np1 - 1 < 20)
                  gr->SetPoint(np1 - 1, xpos, zpos, ypos);
      ...
      double *kinematics =
         kine_2b(proj_mass, target_mass, he2_mass_ex, recoil_mass, 100 * 56, theta_He2 * TMath::DegToRad(), kin_He2);
