...
double *kine_2b(double m1, double m2, double m3, double m4, double K_proj, double thetalab, double K_eject)
{
   ...
   // theta_cm = acos( ( pow(s,2) +s*(2*t - pow(m1,2) - pow(m2,2) - pow(m3,2) - pow(m4_ex,2)) + (pow(m1,2) -
   // pow(m2,2))*(pow(m3,2) - pow(m4_ex,2)) )/( omega(s,pow(m1,2),pow(m2,2))*omega(s,pow(m3,2),pow(m4_ex,2))) ) ;
   ...
   J_LtoCM = fabs(((omega(s, pow(m1, 2), pow(m2, 2)) * omega(s, pow(m3, 2), pow(m4, 2))) / (4 * s * p1 * p3)) *
                  (1. + Et1 / m2 - cos(thetalab) * (Et3 * p1) / (m2 * p3)));
...
std:
   ...
   for (Int_t iEvent = 0; iEvent < nEvents; iEvent++) {
            ...
            if (rpos > holeradius) {
               for (int elec = 0; elec < 100; elec++) {
                  ...
                  driftTime = samplingtime *
                              (floor((((driftLength_fold / 10) / driftVelocity) + (tTime)) / samplingtime) + 0.5); // us
      ...
      // cout <<energyLoss_p1<<"  "<<range_p1<<"  "<<eLoss_p1_hi<<"  "<<range1_hi<<"  "<<(GetEloss(eLoss_p1_hi,
      // range1_hi, sqrt( sq(xpos1_hi) + sq(ypos1_hi) + sq(zpos1_hi - real_vertex)), X,
      // Y)+eLoss_p1_hi-energyLoss_p1)/energyLoss_p1*100<<endl;

      // cout <<energyLoss_p1<<"  "<<range_p1<<"  "<<eLoss_p1_hi<<"  "<<range1_hi<<"  "<<GetEloss(eLoss_p1_hi,
      // range1_hi, sqrt( sq(xpos1_hi) + sq(ypos1_hi) + sq(zpos1_hi - real_vertex)))<<endl;
      ...
      if (iEvent % 2 != 0 && points1 > 0 && points2 > 0) {
         ...
         if (parFit[2] > lastZ1)
            theta_p1_fit = 180.0 - theta_p1_fit;

         TFitResultPtr res1 = grphi1->Fit("pol1", "Sq");
         Double_t phi_p1_fit =
            atan2(signo(lastX1) * signo(res1->Value(1)) * fabs(res1->Value(1)), signo(lastX1)) * TMath::RadToDeg();
         if (phi_p1_fit < 0)
            phi_p1_fit += 360;
         ...
         // cout<<d2heana->GetThetaFit()<<" "<<d2heana->GetPhiFit()<<" "<<d2heana->GetVertexFit()<<"
         // "<<d2heana->GetFitStatus()<<endl; cout<<endl;
         ...
         // set step sizes different than default ones (0.3 times parameter values)
         for (int i = 0; i < 4; ++i)
            fitter2.Config().ParSettings(i).SetStepSize(0.01);
         ...
         if (parFit2[2] > lastZ2)
            theta_p2_fit = 180.0 - theta_p2_fit;

         TFitResultPtr res2 = grphi2->Fit("pol1", "Sq");
         Double_t phi_p2_fit =
            atan2(signo(lastX2) * signo(res2->Value(1)) * fabs(res2->Value(1)), signo(lastX2)) * TMath::RadToDeg();
         if (phi_p2_fit < 0)
            phi_p2_fit += 360;
         ...
         bool goodfit = false;
         if (ok == true && ok2 == true && (fabs(parFit2[2] - parFit[2]) < 30))
            goodfit = true;
         ...
         if (goodfit == true && (av_dE_p1 > 5 && av_dE_p2 > 5)) {
            ...
            mom_proton1_res.SetX(mom1_norm_res * TMath::Sin(theta_p1_fit * TMath::Pi() / 180) *
                                 TMath::Cos(phi_p1_fit * TMath::Pi() / 180));
            mom_proton1_res.SetY(mom1_norm_res * TMath::Sin(theta_p1_fit * TMath::Pi() / 180) *
            ...
            mom_proton2_res.SetX(mom2_norm_res * TMath::Sin(theta_p2_fit * TMath::Pi() / 180) *
                                 TMath::Cos(phi_p2_fit * TMath::Pi() / 180));
            mom_proton2_res.SetY(mom2_norm_res * TMath::Sin(theta_p2_fit * TMath::Pi() / 180) *
            ...
            double *kinematics_res = kine_2b(proj_mass, target_mass, he2_mass_ex, recoil_mass, Ekin_proj,
                                             theta_He2 * TMath::DegToRad(), kin_He2);
...
cout<<"hallo----------------------------------------------------"<<endl;
      ...
      if (av_dE_p1 > 5 && av_dE_p2 > 5 && iEvent % 2 != 0) {
         ...
         double *kinematics = kine_2b(proj_mass, target_mass, he2_mass_ex, recoil_mass, Ekin_proj,
                                      theta_He2 * TMath::DegToRad(), kin_He2);
