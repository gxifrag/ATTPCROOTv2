...
std:
   ...
   // for(Int_t iEvent=0; iEvent<nEvents; iEvent++)
   {
            ...
            if (rpos > 20) {
               for (int elec = 0; elec < 100; elec++) {
                  ...
                  driftTime = samplingtime *
                              (floor((((driftLength / 10) / driftVelocity) + (tTime)) / samplingtime) + 0.5); // us
      ...
      if (iEvent % 2 != 0 && points1 > 0 && points2 > 0) {
         ...
         // set step sizes different than default ones (0.3 times parameter values)
         for (int i = 0; i < 4; ++i)
            fitter2.Config().ParSettings(i).SetStepSize(0.001);
         ...
         bool goodfit = false;
         if (ok == true && ok2 == true && (fabs(parFit2[2] - parFit[2]) < 10))
            goodfit = true;
         ...
         if (goodfit == true && (av_dE_p1 > 5 && av_dE_p2 > 5)) {
            ...
            mom_proton1_res.SetX(mom1_norm_res * TMath::Sin(theta_p1_fit * PI / 180) *
                                 TMath::Cos(phi_p1_fit * PI / 180));
            mom_proton1_res.SetY(mom1_norm_res * TMath::Sin(theta_p1_fit * PI / 180) *
                                 TMath::Sin(phi_p1_fit * PI / 180));
            ...
            mom_proton2_res.SetX(mom2_norm_res * TMath::Sin(theta_p2_fit * PI / 180) *
                                 TMath::Cos(phi_p2_fit * PI / 180));
            mom_proton2_res.SetY(mom2_norm_res * TMath::Sin(theta_p2_fit * PI / 180) *
                                 TMath::Sin(phi_p2_fit * PI / 180));
            ...
            d2he_obj->kine_2b(proj_mass, target_mass, he2_mass_ex, recoil_mass, 1200, theta_He2 * TMath::DegToRad(),
                              kin_He2);

            double *kinematics_res =
               kine_2b(proj_mass, target_mass, he2_mass_ex, recoil_mass, 1200, theta_He2 * TMath::DegToRad(), kin_He2);
