...
std:
   cout << " Analysis of simulation file  " << mcFileNameBg << endl;
                  ...
                  if(rpos>holeradius){
                     ...
                     if(!track1_flag){
                         ...
                         ypos1_hi = point->GetYIn()*10; //in mm
                                                       //range1_low = sqrt( sq(xpos1_hi) + sq(ypos1_hi) + sq(zpos1_hi -
         real_vertex)); track1_flag = true;

                     }
                     range1_hi = sqrt( sq(xpos1_hi - xpos) + sq(ypos1_hi - ypos) + sq(zpos1_hi - zpos));
                     //eLoss_p1_hi +=  point -> GetEnergyLoss()*1e3;// in MeV
                                              eLoss_p1_hi += d2heana->EnergyFluctuation(point -> GetEnergyLoss()*1e3);//
         in MeV     ;
                  ...
                  if(rpos>holeradius){
                     ...
                     if(!track2_flag){
                         ...
                         ypos2_hi = point->GetYIn()*10; //in mm
                                                       //range2_low = sqrt( sq(xpos2_hi) + sq(ypos2_hi) + sq(zpos2_hi -
         real_vertex)); track2_flag = true;
                  ...
                  random_lenght   = gRandom -> Gaus(0,sigstrlong);
                                      ...
                                      driftTime       = samplingtime*(floor((((driftLength_fold/10)/driftVelocity)
         +(tTime))/samplingtime) + 0.5); //us propZ           = zMesh - driftVelocity*driftTime*10; //mm pBin =
         fPadPlane->Fill(propX,propY,10); padNumber       = pBin-1;
                                 ...
                                 //gr1->SetPoint(counter1,PadCenterCoord[0],propZ,PadCenterCoord[1]);
                                                //if(iEvent==3) cout<<PadCenterCoord[0]<<" "<<propZ<<"
         "<<PadCenterCoord[1]<<endl; counter1++;
                                                      }
                                              if(trackID==3 ){
                        ...
                        //if(iEvent==45) cout<<counter2<<"  "<<driftLength_fold<<" "<<propZ<<" "<<zpos<<endl;
                              ...
                              if(trackID==2  && counter1>0 ){
                                       ...
                                       if(points1==0){ P0[0] = meanX1/counter1; P0[1] = meanY1/counter1; P0[2] =
         meanZ1/counter1;} P1[0] = meanX1/counter1; P1[1] = meanY1/counter1; P1[2] = meanZ1/counter1;
                                       //if(iEvent==73)cout<<points1-1<<"  "<<meanX1/counter1<<"  "<<meanZ1/counter1<<"
         "<<meanY1/counter1<<endl; points1++;
                                             }
                   ...
                   //if(iEvent==73)cout<<points2-1<<"  "<<meanX2/counter2<<"  "<<meanZ2/counter2<<"
         "<<meanY2/counter2<<endl; points2++;
                                             }
      ...
      */
         ...
         if (iEvent % 2 != 0 && VolName == "drift_volume") { // solo los protones
            ...
            if (rpos > holeradius) {
               for (int elec = 0; elec < 100; elec++) {
                  ...
                  random_lenght = gRandom->Gaus(0, sigstrlong);
                  driftLength_fold = driftLength + random_lenght; // mm
                  driftTime = samplingtime *
                              (TMath::Floor(((driftLength_fold / 10) / driftVelocity) / samplingtime) + 0.5); // us
            ...
            if (trackIDBg == 1 && Counter[0] > 0) {
               ...
               if (pointsbg[0] == 0) {
                  Pi1bg[0] = meanXbg[0] / Counter[0];
                  Pi1bg[1] = meanYbg[0] / Counter[0];
                  Pi1bg[2] = meanZbg[0] / Counter[0];
               }
               Pf1bg[0] = meanXbg[0] / Counter[0];
               Pf1bg[1] = meanYbg[0] / Counter[0];
               Pf1bg[2] = meanZbg[0] / Counter[0];
               pointsbg[0]++;
            }
            if (trackIDBg == 2 && Counter[1] > 0) {
               ...
               if (pointsbg[1] == 0) {
                  Pi2bg[0] = meanXbg[1] / Counter[1];
                  Pi2bg[1] = meanYbg[1] / Counter[1];
                  Pi2bg[2] = meanZbg[1] / Counter[1];
               }
               Pf2bg[0] = meanXbg[1] / Counter[1];
               Pf2bg[1] = meanYbg[1] / Counter[1];
               Pf2bg[2] = meanZbg[1] / Counter[1];
               pointsbg[1]++;
            }
            if (trackIDBg == 3 && Counter[2] > 0) {
               ...
               if (pointsbg[2] == 0) {
                  Pi3bg[0] = meanXbg[2] / Counter[2];
                  Pi3bg[1] = meanYbg[2] / Counter[2];
                  Pi3bg[2] = meanZbg[2] / Counter[2];
               }
               Pf3bg[0] = meanXbg[2] / Counter[2];
               Pf3bg[1] = meanYbg[2] / Counter[2];
               Pf3bg[2] = meanZbg[2] / Counter[2];
               pointsbg[2]++;
            }
            if (trackIDBg == 4 && Counter[3] > 0) {
               ...
               if (pointsbg[3] == 0) {
                  Pi4bg[0] = meanXbg[3] / Counter[3];
                  Pi4bg[1] = meanYbg[3] / Counter[3];
                  Pi4bg[2] = meanZbg[3] / Counter[3];
               }
               Pf4bg[0] = meanXbg[3] / Counter[3];
               Pf4bg[1] = meanYbg[3] / Counter[3];
               Pf4bg[2] = meanZbg[3] / Counter[3];
               pointsbg[3]++;
            }
      ...
      if (iEvent % 2 != 0) {
         ...
         if (pointsbg[0] > 0) {
            ...
            if (fabs(guess_x1bg_slope) > 20 || (Pf1bg[1] - Pi1bg[1]) == 0)
               guess_x1bg_slope = 0.2;
            double guess_z1bg_slope = (Pf1bg[2] - Pi1bg[2]) / (Pf1bg[1] - Pi1bg[1]);
            if (fabs(guess_z1bg_slope) > 20 || (Pf1bg[1] - Pi1bg[1]) == 0)
               guess_z1bg_slope = 0.2;
         ...
         if (pointsbg[1] > 0) {
            ...
            if (fabs(guess_x2bg_slope) > 20 || (Pf2bg[1] - Pi2bg[1]) == 0)
               guess_x2bg_slope = 0.2;
            double guess_z2bg_slope = (Pf2bg[2] - Pi2bg[2]) / (Pf2bg[1] - Pi2bg[1]);
            if (fabs(guess_z2bg_slope) > 20 || (Pf2bg[1] - Pi2bg[1]) == 0)
               guess_z2bg_slope = 0.2;
         ...
         if (pointsbg[2] > 0) {
            ...
            if (fabs(guess_x3bg_slope) > 20 || (Pf3bg[1] - Pi3bg[1]) == 0)
               guess_x3bg_slope = 0.2;
            double guess_z3bg_slope = (Pf3bg[2] - Pi3bg[2]) / (Pf3bg[1] - Pi3bg[1]);
            if (fabs(guess_z3bg_slope) > 20 || (Pf3bg[1] - Pi3bg[1]) == 0)
               guess_z3bg_slope = 0.2;
         ...
         if (pointsbg[3] > 0) {
            ...
            if (fabs(guess_x4bg_slope) > 20 || (Pf4bg[1] - Pi4bg[1]) == 0)
               guess_x4bg_slope = 0.2;
            double guess_z4bg_slope = (Pf4bg[2] - Pi4bg[2]) / (Pf4bg[1] - Pi4bg[1]);
            if (fabs(guess_z4bg_slope) > 20 || (Pf4bg[1] - Pi4bg[1]) == 0)
               guess_z4bg_slope = 0.2;
         ...
         Double_t ThetaL[6] = {theta_p1_fit,   theta_p2_fit,   theta_p1bg_fit,
                               theta_p2bg_fit, theta_p3bg_fit, theta_p4bg_fit};
         ...
         bool goodfit = false;
         if (GoodFit[track_i] == true && GoodFit[track_k] == true &&
             (fabs(Vertex.at(track_i) - Vertex.at(track_k)) < 20))
            goodfit = true;
         ...
         if (goodfit == true && (Av_Eloss[track_i] > threseloss && Av_Eloss[track_k] > threseloss)) {
            ...
            if (geofile == "_03atm") {
               ...
               eLoss_p1_reco =
                  Eloss_Hi[track_i] + GetEloss(Eloss_Hi[track_i], RangeHi[track_i], RangeLow[track_i], X, Y);
               eLoss_p2_reco =
                  Eloss_Hi[track_k] + GetEloss(Eloss_Hi[track_k], RangeHi[track_k], RangeLow[track_k], X, Y);
            }

            // reconstruction of 2He
            // Double_t mom1_norm_res = TMath::Sqrt(energyLoss_p1*energyLoss_p1 + 2.0*energyLoss_p1*proton_mass);
            // //e_perfect
            Double_t mom1_norm_res =
               TMath::Sqrt(eLoss_p1_reco * eLoss_p1_reco + 2.0 * eLoss_p1_reco * proton_mass); // e_reco
            mom_proton1_res.SetX(mom1_norm_res * TMath::Sin(ThetaL[track_i] * TMath::Pi() / 180) *
                                 TMath::Cos(PhiL[track_i] * TMath::Pi() / 180));
            mom_proton1_res.SetY(mom1_norm_res * TMath::Sin(ThetaL[track_i] * TMath::Pi() / 180) *
                                 TMath::Sin(PhiL[track_i] * TMath::Pi() / 180));
            ...
            mom_proton2_res.SetX(mom2_norm_res * TMath::Sin(ThetaL[track_k] * TMath::Pi() / 180) *
                                 TMath::Cos(PhiL[track_k] * TMath::Pi() / 180));
            mom_proton2_res.SetY(mom2_norm_res * TMath::Sin(ThetaL[track_k] * TMath::Pi() / 180) *
                                 TMath::Sin(PhiL[track_k] * TMath::Pi() / 180));
      ...
      if (av_dE_p1 > threseloss && av_dE_p2 > threseloss && iEvent % 2 != 0) {
         ...
         mom_proton1.SetX(mom1_norm * TMath::Sin(avetheta_p1 * TMath::Pi() / 180) *
                          TMath::Cos(avephi_p1 * TMath::Pi() / 180));
         mom_proton1.SetY(mom1_norm * TMath::Sin(avetheta_p1 * TMath::Pi() / 180) *
         ...
         mom_proton2.SetX(mom2_norm * TMath::Sin(avetheta_p2 * TMath::Pi() / 180) *
                          TMath::Cos(avephi_p2 * TMath::Pi() / 180));
         mom_proton2.SetY(mom2_norm * TMath::Sin(avetheta_p2 * TMath::Pi() / 180) *
         ...
         // cout<<theta_He2<<" "<<mom1_norm<<" "<<mom2_norm<<" "<<avetheta_p1<<" "<<avetheta_p2<<" "<<avephi_p1<<"
         // "<<avephi_p2<<endl;
