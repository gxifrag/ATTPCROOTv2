...
/*	Double_t FindAngleBetweenTracks(const ROOT::Math::XYZVector& vec1,const ROOT::Math::XYZVector& vec2)
   ...
   positive.
      ...
      // The vertex and the mean time of the track are needed to determine the direction of the track and then add 90
   degrees. Double_t num = vec1.X()*vec2.X() + vec1.Y()*vec2.Y() + vec1.Z()*vec2.Z() ; Double_t den =
   TMath::Sqrt(vec1.X()*vec1.X() + vec1.Y()*vec1.Y() + vec1.Z()*vec1.Z())*TMath::Sqrt(vec2.X()*vec2.X() +
   vec2.Y()*vec2.Y() + vec2.Z()*vec2.Z()); Double_t ang = TMath::ACos(num/den); return ang;
   }
*/
...
void runana_merg_cal_d2He()
{
   ...
   Double_t S800_rf = 0., S800_x0 = 0., S800_x1 = 0., S800_y0 = 0., S800_y1 = 0., S800_E1up = 0., S800_E1down = 0.,
