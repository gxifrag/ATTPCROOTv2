...
void anaClassBg::Loop()
{
   ...
   for (int eventos = 0; eventos < 100000; eventos++) {
      ...
      if ((eventos % 1000) == 0)
         cout << " Event number...." << eventos << endl;
      ...
      bool goodfit = false;
      if (GoodFit[track_i] == true && GoodFit[track_k] == true && (fabs(Vertex.at(track_i) - Vertex.at(track_k)) < 20))
         goodfit = true;

      if (goodfit == true && (Av_Eloss[track_i] > 5 && Av_Eloss[track_k] > 5)) {

         // cout<<"hello!"<<endl;
         Double_t eLoss_p1_reco =
            Eloss_Hi[track_i] + GetEloss(Eloss_Hi[track_i], RangeHi[track_i], RangeLow[track_i], X, Y);
         Double_t eLoss_p2_reco =
