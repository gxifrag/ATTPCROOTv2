void makeDummy_proto(const Int_t nEvents = 10)
{
   ...
   for (Int_t i=0; i<gennum; i++) {
     ...
     //cout << pid[dnum][i] << "\t" << pxl[dnum][i] << "\t" << pyl[dnum][i] << "\t" << pzl[dnum][i] << "\t" <<
   kinl[dnum][i] << endl; primGen->AddGenerator(fIongen[i]);
   }
     ...
     //fIongen[i] = new
   FairParticleGenerator(pdg,1,pionpC[0][i],pionpC[1][i],pionpC[2][i],pionrC[0][i]*1.0E-13,pionrC[1][i]*1.0E-13-21.33,pionrC[2][i]*1.0E-13-3.52);
