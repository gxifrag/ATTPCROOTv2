void run_eve_proto(TString InputDataFile = "output_proto.root",
                   TString OutputDataFile = "output_proto.reco_display.root",
                   TString unpackDir = "/Unpack_GETDecoder2/")
{
   ...
   // eve->SetSaveTextData(); //To save the waveform of an event in a txt file when pressig the button (NB: Draw all
   // pads MUST be enabled as well)
