void run_sim_proto(Int_t nEvents = 10000, TString mcEngine = "TGeant4")
{
   ...
   Double_t NomEnergy = 35.0; // Nominal Energy of the beam: Only used for cross section calculation (Tracking energy is
                              // determined with momentum). Must be consistent with pz
   ...
   ATTPCIonPhaseSpace *ReacDecay = new ATTPCIonPhaseSpace("Phase", &Zp, &Ap, &Qp, mult, &Pxp, &Pyp, &Pzp, &Mass,
                                                          ResEner, z, a, px, py, pz, Bmass, TargetMass);
