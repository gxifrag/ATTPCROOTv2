void run_hierarchical_clustering(Int_t firstEvent = 0, Int_t eventCount = std::numeric_limits<Int_t>::max())
{
   ...
   Int_t const lastEvent =
      (eventCount < (eventsBranch->GetEntries() - firstEvent) ? eventCount
                                                              : (eventsBranch->GetEntries() - firstEvent)) +
      firstEvent - (isSimulation ? 1 : 0);
      ...
      if (isSimulation) {
         ...
         for (size_t j = 0; j < 2; ++j) {
            ...
            for (size_t k = 0; k < events->GetEntries(); ++k) {
               ...
               ATHit hit(0, point.GetXIn() * 10.0, point.GetYIn() * 10.0, point.GetZIn() * 10.0, 0.0);
            ...
            if (j == 0) {
               ...
               std::cout << "  simulated vertex-position: " << vertexPosition.X() << " " << vertexPosition.Y() << " "
                         << vertexPosition.Z() << std::endl;
            }
         }
      } else {
      ...
      try {
         ...
         for (ATTrajectory const &trajectory : trajectories) {
            ...
            for (ATHit const &hit : trajectory.GetHits()) {
               std::cout << "      hit: " << hit.GetTrackID() << " (" << hit.GetPosition().X() << " "
                         << hit.GetPosition().Y() << " " << hit.GetPosition().Z() << ")" << std::endl;
            }
            ...
            float const trajectoryLength =
               cubicSplineFit.CalculateArcLength(splineStartPosition, splineEndPosition, 10000);
            float const averageCurvature =
               cubicSplineFit.CalculateAverageCurvature(splineStartPosition, splineEndPosition, 10000, 0.1f);
            ...
            std::cout << "    startHitPosition: " << startHitPosition.X() << " " << startHitPosition.Y() << " "
                      << startHitPosition.Z() << std::endl;
            std::cout << "    splineEndPosition: " << splineEndPosition << std::endl;
            std::cout << "    endHitPosition: " << endHitPosition.X() << " " << endHitPosition.Y() << " "
                      << endHitPosition.Z() << std::endl;
            ...
            std::cout << "    centroidPoint: " << centroidPoint(0) << " " << centroidPoint(1) << " " << centroidPoint(2)
                      << std::endl;
            std::cout << "    mainDirection: " << mainDirection(0) << " " << mainDirection(1) << " " << mainDirection(2)
                      << std::endl;
         }
