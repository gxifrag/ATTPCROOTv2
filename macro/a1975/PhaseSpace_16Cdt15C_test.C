...
void PhaseSpace_1n_16Cdt15C_binWidth( int Nbin, double MinLim, double MaxLim, double sigma_Ex, double thetaCM_min, double thetaCM_max, double theta_min, double theta_max , double T_min, double T_max, int SavingDataFile, TString OutputFileName_1n ) {
...
//------------------------------------------------------------------------------
	...
	TH1F *h_PS_1n = new TH1F("h_PS_1n","Phase Space: ^{16}C + d #rightarrow t + ^{14}C + 1n", Nbin, MinLim, MaxLim ) ;
	TH2F *h_kn_1n = new TH2F("h_kn_1n","h_kn: ^{16}C + d #rightarrow t + ^{14}C + 1n", 180, 0,50, 600, 0,60 ) ;

	TH1F *h_theta_debug1n = new TH1F("h_theta_debug1n", "#theta_{lab} de eventos generados", 180, 0, 180);
	TH1F *h_t3_debug1n = new TH1F("h_t3_debug1n", "kinetic energy deuteron", 600, 0, 200);
...
void PhaseSpace_2n_16Cdt15C_binWidth( int Nbin, double MinLim, double MaxLim, double sigma_Ex,double thetaCM_min, double thetaCM_max, double theta_min, double theta_max , double T_min, double T_max, int SavingDataFile, TString OutputFileName_2n ) {
	...
	Double_t masses[4] = {m3, m7, mn, mn};	
//------------------------------------------------------------------------

	// Q-value de la reacción
	double Q = (m1 + md) - (m7 + m3 + 2 * mn);
	cout << "Q-value: " << Q << " MeV" << endl;
...
//------------------------------------------------------------------------------
	...
	TH1F *h_PS_2n = new TH1F("h_PS_2n", "Phase Space:^{16}C + d #rightarrow t + ^{13}C + 2n", Nbin, MinLim, MaxLim);
	TH2F *h_kn_2n = new TH2F("h_kn_2n","h_kn:^{16}C + d #rightarrow t + ^{13}C + 2n", 180, 0, 50, 600, 0, 60 ) ;

	TH1F *h_theta_debug2n = new TH1F("h_theta_debug2n", "#theta_{lab} de eventos generados", 180, 0, 180);
	TH1F *h_t3_debug2n = new TH1F("h_t3_debug2n", "kinetic energy deuteron", 600, 0, 200);
...
void PhaseSpace_16Cdt15C_test() {
	...
	/*TString OutputFileName_1n = "PhaseSpace_16C_dt_1n.root" ;
	PhaseSpace_1n_16Cdt15C_binWidth( Nbin, MinLim, MaxLim, sigma_Ex, thetaCM_min, thetaCM_max, theta_min, theta_max, T_min, T_max, SavingDataFile, OutputFileName_1n  ) ;
	*/
	TString OutputFileName_2n = "PhaseSpace_16C_dt_2n.root" ;
	PhaseSpace_2n_16Cdt15C_binWidth( Nbin, MinLim, MaxLim, sigma_Ex, thetaCM_min, thetaCM_max, theta_min, theta_max, T_min, T_max, SavingDataFile, OutputFileName_2n  ) ;
