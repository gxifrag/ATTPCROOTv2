...
double nc_sum(double *par, int j){
	...
	ncsum = nc_GS[j] + nc_Ex1[j] + nc_Ex2[j] + nc_Ex3[j] + nc_Ex4[j] + nc_Ex5[j] + nc_Ex6[j] + nc_Ex7[j] + nc_Ex8[j] + par[28] * nc_PS_1n[j] + par[29] * nc_PS_2n[j] ; 
...
void chi2(Int_t &npar, Double_t *gin, Double_t &result, Double_t *par, Int_t flg){
	...
	for ( int ibin = 0 ; ibin < nbins ; ibin++ ) {
		...
		if(x[ibin] < 3.5 && x[ibin] > -1.5 ){
		      ...
		      if(nc_tot[ibin]==0) err_nc = 1.84 ;
		      if(nc_tot[ibin]==1 && nc_sum(par,ibin) < 1 ) err_nc = 0.827 ;
		      if(nc_tot[ibin]==1 && nc_sum(par,ibin) > 1 ) err_nc = 2.300 ;
		      if(nc_tot[ibin]==2 && nc_sum(par,ibin) < 2 ) err_nc = 1.29  ;
		      if(nc_tot[ibin]==2 && nc_sum(par,ibin) > 2 ) err_nc = 2.64  ;

		      if( nc_tot[ibin] > 2 && nc_sum(par,ibin) > nc_tot[ibin] ) err_nc = sqrt(nc_tot[ibin]) + 1 ;
		      if( nc_tot[ibin] > 2 && nc_sum(par,ibin) < nc_tot[ibin] ) err_nc = sqrt(nc_tot[ibin]) ;
...
int Ex_fit_16Cpd15C(){ 
	...
	TH1F *h_PS_1n_aux = new TH1F("h_PS_1n_aux","h_PS_1n_aux", NumberBins, Ebin_min, Ebin_max ) ; // This is for the 300 keV bin widt
	...
	TH1F *h_PS_2n_aux = new TH1F("h_PS_2n_aux","h_PS_2n_aux", NumberBins, Ebin_min, Ebin_max ) ; // This is for the 300 keV bin widt
	...
	for ( int k = 0 ; k < N_States ; k++ ) {
		for ( int i = 0 ; i < 3 ; i++ ) {
			...
			if ( k < 5 ) { index_mix = k * 3 + i ; }
			else{          index_mix = k * 3 + i +1 ; }
			 
			
			if ( i == 0 ) {
				bmin[index_mix] = 0;
				bmax[index_mix] = ival[index_mix] * 4 ; 
				if ( bmax[index_mix] < 10 ) bmax[index_mix] = 50 ;
				step[index_mix] = 0.01 ; 
			}
			if ( i == 1 ) {
	...
	for (int i=0; i<npar; i++) {
		...
		cout<<"i= "<< i <<" eplus =  "<< eplus <<"/ eminus =  " << eminus << "/ eparab =  " << eparab << "/ gcc =  " << gcc << endl;
	...
	TString IndexName[20] = { "Ags", "Egs", "Ab1", "Eb1", "Ab2", "Eb2", "Ar6", "Er6", "Amr", "Emr", "Ar2", "Er2", "Ar3", "Er3", "Ar4", "Er4", "Ar5", "Er5", "A1nPS", "A2nPS" } ; 

	for (int i=0; i<10; i++) {
		//cout << " i / j / cov = " << i << " / "  << sqrt( covariance_matrix[i][i] ) << endl;

		for (int j=0; j<10; j++) {
			double corr_coef = abs( covariance_matrix[i][j] / ( sqrt(covariance_matrix[i][i]) * sqrt(covariance_matrix[j][j]) ) ) ;
			if ( j > i && corr_coef > 0.2 ) cout << IndexName[i] << " / " << IndexName[j]  << " / " << corr_coef << endl;
		}
	}
	...
	for(int i=0;i<NumberBinsAux;i++) {
		...
		if ( nc_tot[i] > 0 ) Residuo_array[i]  = 1.0 * ( nc_tot[i] - cont ) / ( sqrt( nc_tot[i] ) ) ;
		else{ Residuo_array[i]  = 0 ; }
...
*/
	...
	for( int i=0 ; i < NumberBins; ++i) {
		if ( h_GS_plot -> GetBinContent( i+1 ) > 0 ) { h_GS_plot_save->SetBinContent( i+1 , h_GS_plot -> GetBinContent( i+1 ) ) ; }
		else{ h_GS_plot_save->SetBinContent( i+1 , 0 ) ; }

		if ( h_Ex1_plot -> GetBinContent( i+1 ) > 0 ) { h_Ex1_plot_save->SetBinContent( i+1 , h_Ex1_plot -> GetBinContent( i+1 ) ) ; }
		else{ h_Ex1_plot_save->SetBinContent( i+1 , 0 ) ; }

		if ( h_Ex2_plot -> GetBinContent( i+1 ) > 0 ) { h_Ex2_plot_save->SetBinContent( i+1 , h_Ex2_plot -> GetBinContent( i+1 ) ) ; }
		else{ h_Ex2_plot_save->SetBinContent( i+1 , 0 ) ; }

		if ( h_Ex3_plot -> GetBinContent( i+1 ) > 0 ) { h_Ex3_plot_save->SetBinContent( i+1 , h_Ex3_plot -> GetBinContent( i+1 ) ) ; }
		else{ h_Ex3_plot_save->SetBinContent( i+1 , 0 ) ; }

		if ( h_Ex4_plot -> GetBinContent( i+1 ) > 0 ) { h_Ex4_plot_save->SetBinContent( i+1 , h_Ex4_plot -> GetBinContent( i+1 ) ) ; }
		else{ h_Ex4_plot_save->SetBinContent( i+1 , 0 ) ; }

		if ( h_Ex5_plot -> GetBinContent( i+1 ) > 0 ) { h_Ex5_plot_save->SetBinContent( i+1 , h_Ex5_plot -> GetBinContent( i+1 ) ) ; }
		else{ h_Ex5_plot_save->SetBinContent( i+1 , 0 ) ; }

		if ( h_Ex6_plot -> GetBinContent( i+1 ) > 0 ) { h_Ex6_plot_save->SetBinContent( i+1 , h_Ex6_plot -> GetBinContent( i+1 ) ) ; }
		else{ h_Ex6_plot_save->SetBinContent( i+1 , 0 ) ; }

		if ( h_FitCurve -> GetBinContent( i+1 ) > 0 ) { h_FitCurve_save->SetBinContent( i+1 , h_FitCurve -> GetBinContent( i+1 ) ) ; }
		else{ h_FitCurve_save->SetBinContent( i+1 , 0 ) ; }

		if ( h_PS_1n_plot -> GetBinContent( i+1 ) > 0 ) { h_PS_1n_plot_save->SetBinContent( i+1 , h_PS_1n_plot -> GetBinContent( i+1 ) ) ; }
		else{ h_PS_1n_plot_save->SetBinContent( i+1 , 0 ) ; }
...
*/
	...
	cout << "   " << endl ;
	/*
	cout << TString::Format("%.2f", ThetaCM_center)  << "   " << TString::Format("%.2f", step_ThetaCM ) << "   " << TString::Format("%.4f", int_GS ) << "   " << TString::Format("%.4f", u_int_GS ) << "   " << TString::Format("%.4f", int_Ex1 ) << "   " << TString::Format("%.4f", u_int_Ex1 ) << "   " << TString::Format("%.4f", int_Ex2 ) << "   " << TString::Format("%.4f", u_int_Ex2 ) << "   " << TString::Format("%.4f", int_Ex3) << "   " << TString::Format("%.4f", u_int_Ex3) << "   " << TString::Format("%.4f", int_Ex5 )<< "   " << TString::Format("%.4f", u_int_Ex5 ) << "   " << TString::Format("%.4f", int_Ex6 ) << "   " << TString::Format("%.4f", u_int_Ex6 ) << "   " << TString::Format("%.4f", int_Ex7 ) << "   " << TString::Format("%.4f", u_int_Ex7 ) << endl ;
	*/

	cout << TString::Format("%.2f", ThetaCM_center)  << "   " << TString::Format("%.2f", step_ThetaCM ) << "   " << TString::Format("%.4f", int_GS ) << "   " << TString::Format("%.4f", u_int_GS ) << "   " << TString::Format("%.4f", int_Ex1 ) << "   " << TString::Format("%.4f", u_int_Ex1 ) << "   " << TString::Format("%.4f", int_Ex2 ) << "   " << TString::Format("%.4f", u_int_Ex2 ) << "   " << TString::Format("%.4f", int_Ex3) << "   " << TString::Format("%.4f", u_int_Ex3) << "   "  << endl ;	
	...
	cout << TString::Format("%.2f", ThetaCM_center)  << "   " << TString::Format("%.2f", step_ThetaCM ) << "   " << TString::Format("%.2f", fitpar[1])  << "   " << TString::Format("%.2f", fitpar[2]) << "   " << 	TString::Format("%.2f", fitpar[4])  << "   " << TString::Format("%.2f", fitpar[5]) << "   " << 	TString::Format("%.2f", fitpar[7])  << "   " << TString::Format("%.2f", fitpar[8]) << "   " << 	TString::Format("%.2f", fitpar[10])  << "   " << TString::Format("%.2f", fitpar[11]) << "   " << 	TString::Format("%.2f", fitpar[17])  << "   " << TString::Format("%.2f", fitpar[18]) << "   " << 	TString::Format("%.2f", fitpar[20])  << "   " << TString::Format("%.2f", fitpar[21]) << "   " << 	TString::Format("%.2f", fitpar[23])  << "   " << TString::Format("%.2f", fitpar[24]) << endl ; 	cout << "   " << endl ;
	cout << "   " << endl ;
	cout << "---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" << endl ;
