...
spec->Search(hexCorr, 2, "", 0.1); // smoothing=2, threshold=10%
delete cps;

int nFound = spec->GetNPeaks(); // actual number of peaks found
Double_t *xpeaks = spec->GetPositionX(); // <-- this line was missing

cout << "Number of peaks found: " << nFound << std::endl;

// Copy and sort safely
std::vector<double> sortedPeaks;
for (int i = 0; i < nFound; ++i) {
    if (std::isfinite(xpeaks[i])) {
        sortedPeaks.push_back(xpeaks[i]);
    }
}
std::sort(sortedPeaks.begin(), sortedPeaks.end());
cout << "Sorted peak positions: ";
for (const auto& peak : sortedPeaks) {
    cout << peak << " ";
}
cout << std::endl;

/*double initParams[16] = {
    450, sortedPeaks[0], 0.2,
    500, sortedPeaks[1], 0.2,
    220, sortedPeaks[2], 0.3,
    50,  sortedPeaks[3], 0.3,
    45,  sortedPeaks[4], 0.3,
    0.00001  // Phase space scaling factor
}; //0.00001
*/

cout << "SORTED PEAKS USED FOR INITIAL PARAMETERS: ";
for (const auto& peak : sortedPeaks) {
    cout << peak << " ";
}
cout << std::endl;
...
double initParams[16] = {400, 0.57,0.218, 
      ...
      220, 3.9, 0.2,
      50, 5.5, 0.2, 
      25, 6.9, 0.2, 0.00001}; //BW: amplitude mean width 0.0001

//TF1* fitResult = FitHexCorr(hexCorr, params, h_PS_1n);


TF1* model = CreateSpectralModelWithPS(h_PS_1n);
...
hexCorr->Fit(model, "RQ"); // "R" para rango, "Q" para modo silencioso
hexCorr->Draw();
delete ctest;
