...
double Rutherford(double theta, double Ekin, double z, double Z)
{
   ...
   return pow(alpha * z * Z, 2) * pow(hbar * c / Ekin, 2) / (16 * pow(TMath::Sin(theta * TMath::DegToRad() / 2), 4)) *
          1E28 * 1E3;
}
...
void C15_pp_def()
{
   ...
   TH1F *AngDistrCMEfficiency =
      new TH1F("AngDistrCMEfficiency", "AngDistrCMEfficiency", 180 / bin_deg, -0.5, 180 - 0.5);
   auto *RatioToRutherford =
      new TH1F("RatioToRutherford", "^{15}C(p,p)^{15}C elastic scattering", 180 / bin_deg, -0.5, 180 - 0.5);
   ...
   Double_t normalization_constant =
      1 / (2 * TMath::Pi() * rho_H2 * 1E-27 * (zmax - zmin) * 100 * NI * bin_deg * TMath::Pi() / 180); // mbarn
   ...
   for (auto filename : filenamesSim) {
      TFile *simFile =
         new TFile("./data/v0.13.0/analysis2/simulation_15C_pp_gs_new/" + filename + "_" + "1H.root", "R");
      ...
      for (int i = 0; i < Tphysics->GetEntries(); i++) {
         ...
         auto [ex_energy, theta_cm] =
            kine_2b(m_C15, m_p, m_b, m_B, elossTableH2->GetEnergy(Ebeam_buff, zPos * 1000), theta_rad, E_ej);
   ...
   for (auto filename : filenames) {
      ...
      // TFile *physicsFile = new TFile("/home/yassid/attpc_spyral_0.13.0/a2091_15C_pp/InterpSolver_root/" + filename +
      // "_" + "1H.root", "R");
      TFile *physicsFile = new TFile("/home/daniel/Desktop/a2091/InterpSolver_root/" + filename + "_" + "1H.root", "R");
      // TFile *physicsFile = new TFile("/home/yassid/attpc_spyral_dev_test/a2091/C15_pp/InterpSolver_root/" + filename
      // + "_" + "1H.root", "R");
