...
kine_2b(Double_t m1, Double_t m2, Double_t m3, Double_t m4, Double_t K_proj, Double_t thetalab, Double_t K_eject)
{
   ...
   double m4_ex = sqrt((cos(thetalab) * omega(s, pow(m1, 2), pow(m2, 2)) * omega(u, pow(m2, 2), pow(m3, 2)) -
   ...
   double theta_cm =
      TMath::Pi() - acos((pow(s, 2) + s * (2 * t - pow(m1, 2) - pow(m2, 2) - pow(m3, 2) - pow(m4_ex, 2)) +
...
void scan_angle_correction()
{
   ...
   for (int run = 104; run <= 189; run++) {
      // Skip known bad runs
      if (run == 111 || run == 121 || run == 148 || run == 149)
         continue;
      ...
      for (size_t ik = 0; ik < kethe_values.size(); ik++) {
         ...
         for (auto &filename : filenames) {
            ...
            for (int i = 0; i < T->GetEntries(); i++) {
               ...
               if (E_ej >= E_ej_max)
                  continue;
               if (zPos_s * 100 < zPos_min_cm || zPos_s * 100 > zPos_max_cm)
                  continue;
         ...
         cout << Form("alpha=%.5f  kethe=%.1f  ->  GS centroid = %+.6f MeV  (err=%.4f)", alpha_test, kethe_test,
   ...
   lat.DrawLatex(0.15, 0.92,
                 Form("Best: #alpha=%.5f, kethe=%.1f MeV, |centroid|=%.4f MeV", best_alpha, best_kethe,
                      TMath::Abs(best_centroid)));
   ...
   TLine *zero1 = new TLine(h_alpha_proj->GetXaxis()->GetXmin(), 0, h_alpha_proj->GetXaxis()->GetXmax(), 0);
   ...
   TLine *zero2 = new TLine(h_kethe_proj->GetXaxis()->GetXmin(), 0, h_kethe_proj->GetXaxis()->GetXmax(), 0);
