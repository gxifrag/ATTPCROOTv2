...
void C16_pt_ana_inicial()
{
   ...
   for (auto filename : filenames) {
      TFile *runFile =
         ...
         new TFile(
            "/home/georgina/C16_analysis/C16_H2/output_a1975_22April_tb510_MMG20/InterpSolver/InterpSolver_pt_root/" +
               filename,
            "R");
      ...
      for (int i = 0; i < Tphysics->GetEntries(); i++) {
         ...
         Double_t Ebeam_at_z =
            elossH2.GetEnergy(Ebeam_buff, dist3D); // Usar la distancia 3D para la corrección de energía
   ...
   std::vector<std::string> labels = {"Ground State", "1st Excited State"};
   ...
   TF1 *fModel = new TF1("fModel", model, -2., 13.5, 13, // number fitted parameters //13
                         "SpectralModel");
