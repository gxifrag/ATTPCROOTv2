...
#include <iostream>
   ...
   double Ebin_max = 13.5 ;
	double Ebin_min = -2.0 ;
   int NumberBins = 120; //140
...
if (guardar_en_pdf) {
   ...
   auto *hexCorr2 = new TH1F("hexCorr2", "C16(p,t)", NumberBins, Ebin_min, Ebin_max);
   ...
   Double_t m_p   = 938.272081   / 1.0;  // masa nuclear del protón
   Double_t m_t   = 2808.921112  / 1.0;  // masa nuclear del tritón (3H)
   Double_t m_C14 = 13040.870    / 1.0;  // masa nuclear del 14C
   ...
   filenames.push_back("run_0106_3H.root");
   filenames.push_back("run_0107_3H.root");
   ...
   // filenames.push_back("run_0121_3H.root");
   ...
   for (auto filename : filenames) {
      ...
      new TFile("/home/georgina/C16_analysis/C16_H2/output_a1975_22April_tb510_MMG20/InterpSolver/InterpSolver_pt_root/" + filename, "R");
      ...
      for (int i = 0; i < Tphysics->GetEntries(); i++) {
         ...
         Double_t Ebeam_at_z = elossH2.GetEnergy(Ebeam_buff, dist3D); // Usar la distancia 3D para la corrección de energía
      ...
      //if (theta_lab_corr* TMath::RadToDeg()  >17.0){
         ...
         if (zPos*100> 2.0 && zPos*100 < 60.0 && E_ej < 25.0) { //25
           //double ex_energy_corr_shifted = ex_energy_corr - 0.55;
      ...
      //}
         ...
         if (ex_energy_corr_shifted > energyCutGS_lower && ex_energy_corr_shifted < energyCutGS_upper &&
            ...
            hGS_ExEnergy->Fill(ex_energy_corr_shifted);
           
         }

         if(ex_energy_corr_shifted > 0.4 && ex_energy_corr_shifted< 1.2 &&
             theta_cm_corr > theta_deg_lower && theta_cm_corr < theta_deg_upper) {
            Double_t theta_deg = theta_cm_corr;
            h740_AngularDistr->Fill(theta_deg);
            h740_ExEnergy->Fill(ex_energy_corr_shifted);
         }

         if(ex_energy_corr_shifted > 3.0 && ex_energy_corr_shifted< 3.8 &&
             theta_cm_corr > theta_deg_lower && theta_cm_corr < theta_deg_upper) {
            Double_t theta_deg = theta_cm_corr;
            h3103_AngularDistr->Fill(theta_deg);
            h3103_ExEnergy->Fill(ex_energy_corr_shifted);
         }


         if(ex_energy_corr_shifted > 4.0 && ex_energy_corr_shifted< 5.5 &&
             theta_cm_corr > theta_deg_lower && theta_cm_corr < theta_deg_upper) {
            Double_t theta_deg = theta_cm_corr;
            h4780_AngularDistr->Fill(theta_deg);
            h4780_ExEnergy->Fill(ex_energy_corr_shifted);
         }

         if(ex_energy_corr_shifted > 6.0 && ex_energy_corr_shifted< 8.0 &&
             theta_cm_corr > theta_deg_lower && theta_cm_corr < theta_deg_upper) {
            Double_t theta_deg = theta_cm_corr;
            h6841_AngularDistr->Fill(theta_deg);
            h6841_ExEnergy->Fill(ex_energy_corr_shifted);
         }*/
...
//-------------------- PHASE SPACE ----------------------------------------------
   ...
   TCanvas* c_PS_1n = new TCanvas("c_PS_1n","c_PS_1n",800,600) ;
   c_PS_1n -> cd() ;
   h_PS_1n -> SetLineColor(kBlue+2) ;
   h_PS_1n -> SetLineWidth(2) ;
   h_PS_1n -> SetTitle("Phase Space for 16C (p,t)13C + 1n") ;
   h_PS_1n -> GetXaxis() -> SetTitle("Excitation Energy (MeV)") ;
   h_PS_1n -> Scale(0.0004);
   h_PS_1n -> Draw("HIST");
...
std::vector<int> colors = {kOrange+7, kBlue};
...
//---------------- Fitting the experimental data ----------------//
   ...
   c_prefits->cd();
   hexCorr->GetYaxis()->SetRangeUser(0, 160);

   // Breit-Wigner (2)
   TF1 *bwprefit2 = new TF1("bw2", "[0]*TMath::BreitWigner(x,[1],[2])", 8.3,9.8);
   TF1 *bwprefit3 = new TF1("bw3", "[0]*TMath::BreitWigner(x,[1],[2])", 9.5, 11.5); 
   ...
   TF1 * gausprefit2 = new TF1("gausprefit2", "gaus(0)", 6.5, 7.5);
...
//--------------------------------------------------------------------------------------------
   ...
   TF1* fModel = new TF1("fModel",
                        model,
                        -2., 13.5,13, //number fitted parameters //13
                        "SpectralModel");
   ...
   fModel->SetParLimits(7, 8.5, 9.); // mean p7
   fModel->SetParLimits(11, 0.5, 1.2); // sigma2  fit global: #8
   fModel->SetParLimits(12, 0., 1.); // phase space fit global: #12
   ...
   TLine *vline0 = new TLine(8.176, 0, 8.176, 130);
   ...
   h_PS_1n->SetLineWidth(4);
   //h_PS_1n->SetMarkerSize(0);
   ...
   TLegend* legend2 = new TLegend(0.65, 0.6, 0.9, 0.9); // (x1, y1, x2, y2) en coordenadas del canvas
   //legend2->SetBorderSize(0); // sin borde
   legend2->SetFillStyle(0);  // fondo transparente
   legend2->SetTextSize(0.03);   // adjust value (default ~0.02)
...
// Create and draw a horizontal line at y=35
TLine *line = new TLine(xMin, 20, xMax, 20);
...
legend->SetTextSize(0.03);   // adjust value (default ~0.02)
legend->SetFillStyle(0);  // fondo transparente
legend->Draw();
