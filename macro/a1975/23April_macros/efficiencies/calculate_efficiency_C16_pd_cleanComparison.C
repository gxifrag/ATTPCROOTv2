...
void calculate_efficiency_C16_pd_cleanComparison()
{
   ...
   for (auto filename : filenames) {
      ...
      for (int i = 0; i < Tphysics->GetEntries(); i++) {
         ...
         auto [ex_tilt, theta_cm_tilt] = kine_2b(m_C16, m_p, m_d, m_C15, Ebeam_at_z, theta_tilt, E_ej);

         // --- SIN tilt (theta crudo, mismo Ebeam_at_z) ---
         auto [ex_notilt, theta_cm_notilt] = kine_2b(m_C16, m_p, m_d, m_C15, Ebeam_at_z, theta, E_ej);
