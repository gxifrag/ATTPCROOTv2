...
void calculate_efficiency_C16_pd()
{
 ...
 std::vector<TString> filenames;
   ...
   for (auto filename : filenames) {
      TFile *runFile =
      new TFile("/home/georgina/engine_ExUniform_pd/engine_ExUniform_pd/InterpSolver/InterpSolverRoot/" + filename, "R");
      ...
      for (int i = 0; i < Tphysics->GetEntries(); i++){
         ...
         double kethe= 13.;    
         double theta_lab_corr_tilt= (theta-(2.0*TMath::Pi()/4000) * (E_ej - kethe)); //theta: rad; theta_lab_corr: rad; E_ej-kethe: MeV 29.5
        ...
        double theta_deg = theta * TMath::RadToDeg();
         ...
         hexvstheta_CM->Fill(ex_energy_corr, theta_cm_corr ); //theta_cm_corr is already in degrees, ex_energy_corr is in MeV
         hexvstheta_lab->Fill(ex_energy_corr, theta_lab_corr* TMath::RadToDeg()); //theta_lab_corr_tilt is in radians, convert to degrees for the histogram
...
TFile *fSim = new TFile("/home/georgina/fair_install/ATTPCROOTv2/macro/a1975/23April_macros/efficiencies/rawSim/output_16Cpd.root", "READ");
...
for (int i = 0; i < tSim->GetEntries(); i++) {
   ...
   auto [sim_ex, sim_theta_cm] = kine_2b(m_C16, m_p, m_d, m_C15, 
                                          sim_Ebeam_at_z, sim_theta_lab, sim_ke);
...
int nSim_total = tSim->GetEntries() / 4; // 4 particulas por evento, raw simulation sin detector, entonces el número total de eventos es el número de entradas dividido por 4
