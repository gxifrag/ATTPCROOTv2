...
kine_2b(Double_t m1, Double_t m2, Double_t m3, Double_t m4, Double_t K_proj, Double_t thetalab, Double_t K_eject)
{
   ...
   /*cout << "m1 = " << m1 << " MeV/c^2" << "m2" << m2 << " MeV/c^2" << "m3" << m3 << " MeV/c^2" << "m4" << m4 << " MeV/c^2" << endl;
   cout << "K_proj = " << K_proj << " MeV" << "thetalab = " << thetalab << " radianes" << "K_eject = " << K_eject << " MeV" << endl;*/
...
void C16_pd_ana_v16_Eff()
{
   ...
   // Cargar eficiencia
   TFile *fEff = TFile::Open("/home/georgina/fair_install/ATTPCROOTv2/macro/a1975/23April_macros/efficiencies/efficiency.root", "READ");
 ...
 // -----------------------------KINEMATICS FOR DIFFERENT EXCITATION ENERGIES
   ...
   std::vector<std::string> labels = {
      ...
      "3rd Excited State", "4th Excited State"
   };
   ...
   for (auto filename : filenames) {
      TFile *runFile =
      new TFile("/home/georgina/C16_analysis/C16_H2/output_a1975_22April_tb510_MMG20/InterpSolver/InterpSolver_pd_root/" + filename, "R");
      ...
      Tphysics->SetBranchAddress("vertex_z", &zPos); //zPos is in meters, I will convert it to cm when filling the histograms
      Tphysics->SetBranchAddress("ke", &ke); //ke is in MeV
      Double_t vx_pos{}, vy_pos{}; //vx_pos and vy_pos are in meters, I will convert them to cm when filling the histograms
      ...
      for (int i = 0; i < Tphysics->GetEntries(); i++) {
         ...
         Double_t Ebeam_at_z = elossH2.GetEnergy(Ebeam_buff, dist3D*10.0); // Usar la distancia 3D para la corrección de energía, MeV/mm
               
         //Corrección cinemática
         double kethe= 13.;    
         double theta_lab_corr_tilt= theta - (2.0*TMath::Pi()/4000) * (E_ej - kethe); //corrección lineal de ángulo, con pendiente de 2pi/4000 rad/MeV, y con un offset de 13 MeV (ke-the) para que la corrección sea cero en ke=13 MeV. Este valor de ke-the lo elegí porque es el valor de ke donde veo que la corrección de energía hace que ex_energy_corr_tilt sea igual a ex_energy sin corrección, es decir, donde la corrección de energía no tiene efecto en el cálculo de ex_energy, por lo que la corrección de ángulo también debería ser cero para que ex_energy_corr_tilt sea igual a ex_energy sin corrección.
      
         double theta_lab_corr = theta; // Por ahora sin corrección de ángulo, solo para probar la implementación de la corrección de energía en el cálculo de Ex y theta_cm
         auto [ex_energy_corr, theta_cm_corr]= kine_2b(m_C16, m_p, m_b, m_B, Ebeam_at_z, theta_lab_corr, E_ej);

         auto [ex_energy_corr_tilt, theta_cm_corr_tilt] = kine_2b(m_C16, m_p, m_b, m_B, 
                                                         Ebeam_at_z, theta_lab_corr_tilt, 
        ...
        // cout << "theta_cm_deg" << theta_cm_deg << "ex_ener_corr_tilt" << ex_energy_corr_tilt << endl;
         ...
         if (theta_cm_deg < 20 || theta_cm_deg > 160) continue;
         ...
         hexvstheta_CM->Fill(ex_energy_corr_tilt, theta_cm_corr_tilt ); //theta_cm_corr_tilt is already in degrees, ex_energy_corr_tilt is in MeV
         hexvstheta_lab->Fill(ex_energy_corr_tilt, theta_lab_corr_tilt * TMath::RadToDeg()); //theta_lab_corr_tilt is in radians, convert to degrees for the histogram
...
//---------------- Fitting the experimental data ----------------//
   ...
   if (nfound < 5) {
       std::cout << "[AVISO]: Solo encontré " << nfound 
                 << " picos de forma automática. Usando valores de respaldo.\n";
   }
  ...
  SpectralModel* model = new SpectralModel(graphPS);
   TF1* fModel = new TF1("fModel",
                        model,
                        -0.5, 8.0,16, //number fitted parameters
                        "SpectralModel");
...
if (!graphPS)     { std::cerr << "graphPS is null\n"; return; }
   ...
   TLine *line1 = new TLine(Ang_Ener_Corr->GetXaxis()->GetXmin(), 14, 
                           Ang_Ener_Corr->GetXaxis()->GetXmax(), 14);
   ...
   TLine *line2 = new TLine(Ang_Ener_Corr->GetXaxis()->GetXmin(), 20, 
...
out << "GS\t"    << fModel->GetParameter(1) << "\t" << fModel->GetParError(1) << "\t" << fModel->GetParameter(2) << "\t" << fModel->GetParError(2) << endl;
out << "1st\t"   << fModel->GetParameter(4) << "\t" << fModel->GetParError(4) << "\t" << fModel->GetParameter(5) << "\t" << fModel->GetParError(5) << endl;
out << "2nd\t"   << fModel->GetParameter(7) << "\t" << fModel->GetParError(7) << "\t" << fModel->GetParameter(8) << "\t" << fModel->GetParError(8) << endl;
out << "3rd\t"   << fModel->GetParameter(10) << "\t" << fModel->GetParError(10) << "\t" << fModel->GetParameter(11) << "\t" << fModel->GetParError(11) << endl;
out << "4th\t"   << fModel->GetParameter(13) << "\t" << fModel->GetParError(13) << "\t" << fModel->GetParameter(14) << "\t" << fModel->GetParError(14) << endl;   
