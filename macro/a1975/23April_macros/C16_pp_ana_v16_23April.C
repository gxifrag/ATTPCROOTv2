  ...
  for (size_t i = 0; i < angularBins.size(); ++i) {
   ...
   for (auto filename : filenames) {
      TFile *runFile =
         ...
         new TFile(
            "/home/georgina/C16_analysis/C16_H2/output_a1975_22April_tb510_MMG20/InterpSolver/InterpSolver_pp_root/" +
               filename,
            "R");
   ...
   /*for (size_t i = 0; i < files.size(); i++) {
      TString fileKine = Form("/home/georgina/fair_install/ATTPCROOTv2/macro/Kinematics/Decay_kinematics/%s",
   files[i].c_str()); std::ifstream kineStr(fileKine.Data());
...
if (!graphPS)     { std::cerr << "graphPS is null\n"; return; }
   ...
   de fase espacio cout << "Phase Space scale factor (initial): " << globalParamsIni[15] << endl; cout << "Phase Space
   scale factor (final): " << globalParamsFinals[15] << endl; cout << "PS max = " << hPS_final->GetMaximum() << endl;
   }
