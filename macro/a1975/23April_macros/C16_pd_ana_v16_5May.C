...
void C16_pd_ana_v16_5May()
{
   ...
   TH2F *Ang_Ener_tiltCorr = new TH2F("Ang_Ener_tiltCorr", "Ang_Ener_tiltCorr", 720, 10, 40, 1000, 0, 60.0); 
   ...
   // Cargar eficiencia
   /*TFile *fEff = new TFile("/home/georgina/fair_install/ATTPCROOTv2/macro/a1975/23April_macros/efficiencies/efficiency.root", "READ");
   TGraphErrors *gEff = (TGraphErrors*)fEff->Get("gEfficiency");*/
 ...
 // -----------------------------KINEMATICS FOR DIFFERENT EXCITATION ENERGIES
   ...
   for (auto filename : filenames) {
      ...
      for (int i = 0; i < Tphysics->GetEntries(); i++) {
        ...
        double kethe= 13.;    
        double theta_lab_corr_tiltCorr=(theta-(2.0*TMath::Pi()/4000) * (E_ej - kethe)); //theta: rad; theta_lab_corr: rad; E_ej-kethe: MeV 29.5
        ...
        auto [ex_energy_corr, theta_cm_corr]= kine_2b(m_C16, m_p, m_b, m_B, Ebeam_at_z, theta_lab_corr, E_ej);
         ...
         auto [ex_energy_corr_tiltCorr, theta_cm_corr_tiltCorr] = kine_2b(m_C16, m_p, m_b, m_B, 
                                                         Ebeam_at_z, theta_lab_corr_tiltCorr, 
                                                         E_ej); //energies: MeV, angles: radians

         // Convertir theta_cm a grados para evaluar la eficiencia
         double theta_cm_deg = theta_cm_corr_tiltCorr; // theta_cm_corr_tiltCorr ya está en grados según la función kine_2b

         hex_noEff->Fill(ex_energy_corr_tiltCorr); // Llenar el histograma sin corregir por eficiencia, sin cortes, "RAW"
        ...
        double theta_deg = theta * TMath::RadToDeg();
         ...
         hexvstheta_CM->Fill(ex_energy_corr_tiltCorr, theta_cm_corr_tiltCorr ); //theta_cm_corr_tiltCorr is already in degrees, ex_energy_corr_tiltCorr is in MeV
         hexvstheta_lab->Fill(ex_energy_corr_tiltCorr, theta_lab_corr_tiltCorr * TMath::RadToDeg()); //theta_lab_corr_tiltCorr is in radians, convert to degrees for the histogram
...
//---------------- Fitting the experimental data ----------------//
  ...
  SpectralModel* model = new SpectralModel(graphPS);
   TF1* fModel = new TF1("fModel",
                        model,
                        -0.5, 9.0,16, //number fitted parameters
                        "SpectralModel");
...
if (!graphPS)     { std::cerr << "graphPS is null\n"; return; }
  ...
  // hexCorr2->Draw("E1"); //E1
   ...
   TLine *line2 = new TLine(Ang_Ener_Corr->GetXaxis()->GetXmin(), 14, 
