...
void C16_pd_ana_v16_5May()
{
   ...
   // Cargar eficiencia
   /*TFile *fEff = new
   TFile("/home/georgina/fair_install/ATTPCROOTv2/macro/a1975/23April_macros/efficiencies/efficiency.root", "READ");
   ...
   for (auto filename : filenames) {
      ...
      for (int i = 0; i < Tphysics->GetEntries(); i++) {
         ...
         double kethe = 13.;
         double theta_lab_corr_tiltCorr =
            (theta -
   ...
   if (nfound < 5) {
      cout << "[WARNING] TSpectrum no encontró los 5 picos. Usando valores por defecto para los faltantes." << endl;
   }
