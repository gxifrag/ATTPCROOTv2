...
TH2F *h_residual = new TH2F("h_residual", 
                             "#DeltaE vs E_{ej} for GS events", 
                             100, 0, 60,    // E_ej axis
                             100, -20, 20);   // E_meas - E_theory

TH2F *h_dtheta = new TH2F("h_dtheta",
                            "#Delta#theta vs E_{ej} for GS events",
                            100, 0, 60,      // E_ej
                            100, -0.1, 0.1); // theta residual in rad

TH2F *h_thetacorr_vs_Eej = new TH2F("h_thetacorr_vs_Eej",
                                      "#theta_{lab,corr} vs E_{ej}",
                                      100, 0, 14,        // E_ej (MeV)
                                      100, 0, 60);        // theta_lab_corr (deg)


TH2F *h_theta_vs_Eej = new TH2F("h_theta_vs_Eej",
                                      "#theta_{lab} vs E_{ej}",
                                      100, 0, 14,        // E_ej (MeV)
   ...
   TH2F *h_GS_zEx = new TH2F("h_GS_zEx", "GS: Ex vs z", 
                            50, 0, 1.0,   // z en metros
                            50, -0.5, 0.5); // Ex en MeV

   // Perfil de Ex vs z para el 2nd excited (Ex entre 2.5 y 4.5 MeV)  
   TH2F *h_2nd_zEx = new TH2F("h_2nd_zEx", "1st Excited: Ex vs z",
                              50, 0, 1.0,
                              50, 0.5, 1.5);
  ...
  for (size_t i = 0; i < angularBins.size(); ++i) {
   ...
   for (auto filename : filenames) {
      ...
      for (int i = 0; i < Tphysics->GetEntries(); i++) {
         ...
         Double_t Ebeam_at_z = elossH2.GetEnergy(Ebeam_buff, dist3D); // Usar la distancia 3D para la corrección de energía, MeV/mm
               
        //Corrección cinemática
        double kethe= 13.;    
        double theta_lab_corr_JR=(theta-(2.0*TMath::Pi()/4000) * (E_ej - kethe)); //theta: rad; theta_lab_corr: rad; E_ej-kethe: MeV 29.5
         ...
         auto [ex_energy_corr_JR, theta_cm_corr_JR] = kine_2b(m_C16, m_p, m_b, m_B, 
                                                         Ebeam_at_z, theta_lab_corr_JR, 
         ...
         Ang_Ener_Corr->Fill(theta_lab_corr_JR* TMath::RadToDeg(), E_ej); //theta lab!! -> I still have to implement the correction of catima?
