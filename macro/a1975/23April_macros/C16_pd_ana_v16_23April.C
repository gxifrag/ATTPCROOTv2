...
if (guardar_en_pdf) {
   ...
   TH2F *h_2nd_zEx = new TH2F("h_2nd_zEx", "2nd Excited: Ex vs z",
                              50, 0, 1.0,
                              50, 2.5, 4.5);
      ...
      for (int i = 0; i < Tphysics->GetEntries(); i++) {
        ...
        //Corrección cinemática
         ...
         if (zPos*100> 2.0 && zPos*100 < 60.0 && E_ej <20.0){
            if (ex_energy_corr > 0 && ex_energy_corr < 1){
         ...
         if(ex_energy_corr > -0.5 && ex_energy_corr < 1.0)
            h_GS_zEx->Fill(zPos*100, ex_energy_corr);
         if(ex_energy_corr > 3. && ex_energy_corr < 4.5)
...
//---------------- Fitting the experimental data ----------------//
   ...
   /*TF1 *bwprefit1 = new TF1("bw1", "[0]*TMath::BreitWigner(x,[1],[2])", 2.8, 4.3);
   TF1 *bwprefit2 = new TF1("bw2", "[0]*TMath::BreitWigner(x,[1],[2])", 4.8, 6.5);
   TF1 *bwprefit3 = new TF1("bw3", "[0]*TMath::BreitWigner(x,[1],[2])", 7.0, 8.5);*/
