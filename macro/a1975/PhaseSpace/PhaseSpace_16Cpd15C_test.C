...
double Ex_calculation( double ELab, double ThetaLab, double mass_in_MeV[4] , double T_b_A ) { //thetalab: degrees, Tb=11,5MeV/A
...
void PhaseSpace_1n_16Cpd15C_binWidth( int Nbin, double MinLim, double MaxLim, double sigma_Ex, double thetaCM_min, double thetaCM_max, double theta_min, double theta_max , double T_min, double T_max, int SavingDataFile, TString OutputFileName_1n ) {

	if (!gROOT->GetClass("TGenPhaseSpace")) gSystem->Load("libPhysics");

	//Change masses + beam kinetic energy 
	Double_t m1,m2,m3,m4,m6,mn,bE,T1,p1,m5,md;
...
//------------------------------------------------------------------------------
	...
	TH1F *h_PS_1n = new TH1F("h_PS_1n","Phase Space: ^{16}C + p #rightarrow d + ^{14}C + 1n", Nbin, MinLim, MaxLim ) ;
	TH2F *h_kn_1n = new TH2F("h_kn_1n","h_kn: ^{16}C + p #rightarrow d + ^{14}C + 1n", 180, 0,50, 600, 0,60 ) ;

	TH1F *h_theta_debug1n = new TH1F("h_theta_debug", "#theta_{lab} de eventos generados", 180, 0, 180);
	TH1F *h_tD_debug1n = new TH1F("h_tD_debug", "kinetic energy deuteron", 600, 0, 100);
	...
	for (Int_t n=0;  n<Nsim;n++) {
		...
		TLorentzVector *pDeuteron = event.GetDecay(0);
		TLorentzVector *p14C      = event.GetDecay(1);
		TLorentzVector *pNeutron  = event.GetDecay(2);

		e3n = pDeuteron->Energy(); //From Lorentz vector
		t3= e3n-md ; // MeV resolution calculated from the 3 alpha
	...
	TCanvas *C_tD_debug = new TCanvas("C_tD_debug", "Energía cinética del deuterón", 800, 600);
	h_tD_debug1n->GetXaxis()->SetTitle("T_{d} [MeV]");
	h_tD_debug1n->Draw("hist");

	TCanvas *C_theta_debug = new TCanvas("C_theta_debug", "theta", 800, 600);
...
void PhaseSpace_2n_16Cpd15C_binWidth( int Nbin, double MinLim, double MaxLim, double sigma_Ex,double thetaCM_min, double thetaCM_max, double theta_min, double theta_max , double T_min, double T_max, int SavingDataFile, TString OutputFileName_2n ) {

	if (!gROOT->GetClass("TGenPhaseSpace")) gSystem->Load("libPhysics");

	//Change masses + beam kinetic energy 
	Double_t m1,m2,m3,m4,m6,m7,mn,md,bE,T1,p1,m5;
	...
	Double_t masses[4] = {md, m7, mn, mn};	
//------------------------------------------------------------------------

	// Q-value de la reacción
	double Q = (m1 + m2) - (md + m7 + 2 * mn);
	cout << "Q-value: " << Q << " MeV" << endl;
...
//------------------------------------------------------------------------------
	...
	for (int i = 0; i < 4; ++i) {
    cout << "Decay " << i << " mass: " << event.GetDecay(i)->M() << endl;
	}

	double M_final = md + m7 + 2*mn;
	cout << "W mass = " << W.M() << ", sum of final masses = " << M_final << endl;
	...
	TH1F *h_PS_2n = new TH1F("h_PS_2n", "Phase Space:^{16}C + p #rightarrow d + ^{13}C + 2n", Nbin, MinLim, MaxLim);
	TH2F *h_kn_2n = new TH2F("h_kn_2n","h_kn:^{16}C + p #rightarrow d + ^{13}C + 2n", 180, 0, 50, 600, 0, 60 ) ;
	...
	for (Int_t n=0;  n<Nsim;n++) {
		...
		theta_deg = pDeuteron -> Theta() * 180. / TMath::Pi() ;  
		Ex_and_ThetaCM_calculation_v2( t3, theta_deg, mass_in_MeV, bE, Ex_ThetaCM ) ;
...
void PhaseSpace_16Cpd15C_test() {
	...
	PhaseSpace_1n_16Cpd15C_binWidth( Nbin, MinLim, MaxLim, sigma_Ex, thetaCM_min, thetaCM_max, theta_min, theta_max, T_min, T_max, SavingDataFile, OutputFileName_1n  ) ;
	
	/*TString OutputFileName_2n = "PhaseSpace_16C_pd_2n.root" ;
	PhaseSpace_2n_16Cpd15C_binWidth( Nbin, MinLim, MaxLim, sigma_Ex, thetaCM_min, thetaCM_max, theta_min, theta_max, T_min, T_max, SavingDataFile, OutputFileName_2n  ) ;
*/
}
