...
if (guardar_en_pdf) {
   ...
   TH2F *Ang_Ener_Corr_Cut = new TH2F("Ang_Ener_Corr_Cut", "Ang_Ener_Corr_Cut",720, 10, 60, 1000, 0, 60.0);
      ...
      for (int i = 0; i < Tphysics->GetEntries(); i++) {
        ...
        //Correccion cinematica
         ...
         if (zPos*100> 2.0 && zPos*100 < 60.0 && ke <25.0){
            ...
            Ang_Ener_Corr_Cut->Fill(theta_lab_corr* TMath::RadToDeg(), ke); //
         }
...
//---------------- Fitting the experimental data ----------------//
   ...
   TF1 *bwprefit3 = new TF1("bw3", "[0]*TMath::BreitWigner(x,[1],[2])", 6., 7.5); //6.2, 8.
   ...
   bwprefit3->SetParameters(60,m5,0.5);
...
/*TCanvas *c_AngEner_Corr = new TCanvas("AngEnerCorr", "Energy as a function of #theta_Corr", 800, 1200);
Ang_Ener_Corr_Cut->SetMarkerStyle(20);
Ang_Ener_Corr_Cut->SetMarkerSize(0.5);
Ang_Ener_Corr_Cut->Draw("col");
Ang_Ener_Corr_Cut->GetXaxis()->SetTitle("#theta_lab (deg)");
Ang_Ener_Corr_Cut->GetYaxis()->SetTitle("Energy (MeV)"); //Energy (MeV)
...
// Optional: add a legend
auto legend0 = new TLegend(0.45, 0.7, 0.9, 0.9);
for (size_t i = 0; i < graphs.size(); i++) {
