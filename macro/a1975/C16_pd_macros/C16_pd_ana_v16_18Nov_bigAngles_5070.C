...
//---------------------------main function---------------------------------------
void C16_pd_ana_v16_18Nov_bigAngles_5070()
{
...
//---------------- Fitting the experimental data ----------------//
  ...
  /* double m1 = sorted_peaks[0];
   ...
   // Breit-Wigner 1
   TF1 *bwprefit1 = new TF1("bw1", "[0]*TMath::BreitWigner(x,[1],[2])", 3., 4.);
   TF1 *bwprefit2 = new TF1("bw2", "[0]*TMath::BreitWigner(x,[1],[2])", 4.,5.2); //2.5,3.9
   TF1 *bwprefit3 = new TF1("bw3", "[0]*TMath::BreitWigner(x,[1],[2])", 6.5, 7.5); //6., 8.
...
fModel->FixParameter(14,bwprefit3->GetParameter(2));//14
...
if (!g_PS)     { std::cerr << "g_PS is null\n"; return; }
   ...
   hexCorr2->SetStats(0);
   hexCorr2->SetTitle("Angular range: (50-70) deg");  // removes the histogram title
