        ...
        //Correccion cinematica
         ...
         if (zPos*100> 2.0 && zPos*100 < 60.0 && ke <25.0){
           double ex_energy_corr_shifted = ex_energy_corr - 0.55;
            ExCorrvsZpos->Fill(ex_energy_corr_shifted, zPos*100.0);
            hexCorr->Fill(ex_energy_corr_shifted); //-0.5 MeV shift to match the known C15 gs energy
            hexCorr2->Fill(ex_energy_corr_shifted); //-0.5 MeV shift to match the known C15 gs energy
...
//---------------- Fitting the experimental data ----------------//
  ...
  SpectralModelSeg* model = new SpectralModelSeg(g_PS);
   TF1* fModel = new TF1("fModel",
                        model,
                        -1., 9.,16, //number fitted parameters
                        "SpectralModelSeg");
