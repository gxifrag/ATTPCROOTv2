void run_KineFitting()
{
   ...
   // Deuteron
   // Ex1
   // 3D position: TVector3 A 3D physics vector (x,y,z)=(0.154338,-0.475520,39.808893)
   // (rho,theta,phi)=(39.812032,0.719511,-72.018279) 3D momentum: TVector3 A 3D physics vector
   // (x,y,z)=(0.032863,-0.133326,0.136415) (rho,theta,phi)=(0.193558,45.188715,-76.153215)
   // Ex2
   // 3D position: TVector3 A 3D physics vector (x,y,z)=(1.383145,-0.035513,37.884856)
   // (rho,theta,phi)=(37.910113,2.091582,-1.470772) 3D momentum: TVector3 A 3D physics vector
   // (x,y,z)=(0.114004,-0.010285,0.107284) (rho,theta,phi)=(0.156885,46.855305,-5.155180)

   // 10Be
   // Ex 1
   //  3D position: TVector3 A 3D physics vector (x,y,z)=(-0.057949,0.478828,43.106549)
   //  (rho,theta,phi)=(43.109247,0.641060,96.900540) 3D momentum: TVector3 A 3D physics vector
   //  (x,y,z)=(-0.021864,0.136026,1.214899) (rho,theta,phi)=(1.222686,6.469810,99.131283)
   // Ex 2
   // 3D position: TVector3 A 3D physics vector (x,y,z)=(-0.442648,0.047374,40.946464)
   // (rho,theta,phi)=(40.948884,0.622903,173.891280)
   // 3D momentum: TVector3 A 3D physics vector (x,y,z)=(-0.116263,0.013200,1.256397)
   // (rho,theta,phi)=(1.261834,5.320676,173.522828)
   ...
   Double_t eloss =
      0.11269 * vertex -
      0.05228; // adhoc function to get the energy at the vertex of a 10Be beam at 10A MeV in 600 torr D2. In cm
   ...
   std::vector<Double_t> parameters{beamMom.X(), beamMom.Y(), beamMom.Z(), beamEtot,   momSca.X(), momSca.Y(),
                                    momSca.Z(),  EScaTot,     momRec.X(),  momRec.Y(), momRec.Z(), ERecTot};
