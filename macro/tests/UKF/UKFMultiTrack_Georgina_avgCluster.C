...
int mat_A = 1;               // H-1
double density_300torr = 3.553e-5; //3.3084e-5;  // g/cm^3 (tu valor para 300 torr)
...
std::map<int, std::vector<ROOT::Math::XYZVector>> trueMomAtHit; // To store the true momentum vector at each hit for each track ID

// To save the XYZVector of the initial momentum for each track
std::map<int, ROOT::Math::XYZVector> initialMom;

int pointsToCluster= 5; //10 protons
...
UKFResult runKalman(const std::vector<double>& hX, const std::vector<double>& hY, const std::vector<double>& hZ, const std::vector<double>& hEloss,
                    const std::vector<ROOT::Math::XYZVector>& hTrueMomVector,
                    double mass, double charge, int Z, int A, ROOT::Math::XYZVector initialMom, int trackID, bool drawPlots = true)
{
  ...
  // XYZVector startMom = initialMom.R() * (nextPos - startPos).Unit(); 
   ...
   // Enable energy straggling for more realistic modeling of energy loss fluctuations
   ukf.fEnableEnStraggling = true; //true; 
   ukf.setParameters(1e-3, 2, 0);
   // alpha = 1e-3, beta = 2, kappa = -3
   //ukf.setParameters(1e-3, 2, -3);
   //ukf.setParameters(1, 2, 0);
   ...
   if (xForward.size() < 4) {
     ...
     // This pass runs backwards along the track to improve the state estimates using all available future data.
        ...
        failResult.res_phi = 999.0;   // Use a special value to indicate fit failure in residuals   
        return failResult;
        }
        ...
        double phi_smoothed = smoothedStates[1][5];
...
//--------------------------------------------------------------------------------------------------------------------
   ...
   result.phi_rec = smoothedStates[1][5];   // Reconstructed azimuthal angle (smoothed) without wrapping, since we will wrap the difference later
   ...
   if (hTrueMomVector.size() > 1) {
       // Compare exactly at the same physical location (the second clustered hit)
       ROOT::Math::XYZVector trueMomAtHit1 = hTrueMomVector[1];
       
       result.p_true = trueMomAtHit1.R(); 
       result.theta_true = trueMomAtHit1.Theta(); 
       result.phi_true = trueMomAtHit1.Phi(); 

       // Calculate residuals
       result.res_theta = result.theta_rec - result.theta_true;
       cout << "DEBUG: Track " << trackID << " - Theta Residual (rec - true) = " << result.res_theta  << " rad" << endl;
       
       // Calculate and WRAP the phi difference exactly once
       double dphi = result.phi_rec - result.phi_true;
       result.res_phi = TVector2::Phi_mpi_pi(dphi);
       cout << "DEBUG: Track " << trackID << " - Phi Residual (rec - true) = " << result.res_phi  << " rad" << endl;
    } else {
       // Fallback just in case the track was too short
       result.p_true = initialMom.R(); 
       result.theta_true = initialMom.Theta(); 
       result.phi_true = initialMom.Phi(); 
