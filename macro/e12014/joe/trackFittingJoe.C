...
void FitTracks(int cutLimit = 150, int tpc_run_num = 211)
{
   ...
   // tpc_tree.Add(TString::Format("/mnt/analysis/hira_collaboration/e12014/Joe/UnpackedRunsDataReduced/run-%d.root",
   // run_num));
      ...
      for (int cut = 0; cut < cutLimit; ++cut) {
         ...
         if (ransac->GetEntries() > 0) {
            ...
            if (validTrackIndex.size() == 2) {
               ...
               // std::cout << "RANSAC Track 1: {(" << p1[0] << ") + (" << d1[0] << ")*z ,(" << p1[1] << ") + (" <<
               // d1[1] << ")*z}"<< std::endl; std::cout << "RANSAC Track 2: {(" << p2[0] << ") + (" << d2[0] << ")*z
               // ,(" << p2[1] << ") + (" << d2[1] << ")*z}"<< std::endl; std::cout << "Vertex at " << vertex << " mm
               // (z)" << std::endl;
               ...
               for (const auto &hit : *hits1) {
                  ...
                  if (sqrt(hit.GetPosition().X() * hit.GetPosition().X() +
                           hit.GetPosition().Y() * hit.GetPosition().Y()) < cut)
                     break;
   ...
   plotVertex->Draw("");
   plotVertex->SetTitle("Vertex Location (z, mm) From Fitting Events Outside Cut Range");
   plotVertex->GetYaxis()->SetTitle("Vertex (z, mm)");
   plotVertex->GetYaxis()->CenterTitle(true);
   plotVertex->GetXaxis()->SetTitle("Cut Radius (mm)");
   plotVertex->GetXaxis()->CenterTitle(true);
}
   ...
   for (int i = 0; i < 5; ++i) {
      ...
      if (ransac->GetEntries() > 0) {
         ...
         if (validTrackIndex.size() == 2) {
            ...
            // Print out the RANSAC fits in parametric form
            // std::cout << "RANSAC Track 1: {(" << p1[0] << ") + (" << d1[0] << ")*z ,(" << p1[1] << ") + (" << d1[1]
            // << ")*z}"<< std::endl; std::cout << "RANSAC Track 2: {(" << p2[0] << ") + (" << d2[0] << ")*z ,(" <<
            // p2[1] << ") + (" << d2[1] << ")*z}"<< std::endl; std::cout << "Vertex at " << vertex << " mm (z)" <<
            // std::endl;
            ...
            for (const auto &hit : *hits1) {
               // Adding cuts
               // Only fit parts of track that are more than 10cm from center of detector
               if (sqrt(hit.GetPosition().X() * hit.GetPosition().X() + hit.GetPosition().Y() * hit.GetPosition().Y()) <
                   cut_radius)
                  break;
...
void FitTracksPlotAngle(int cutLimit = 150, int tpc_run_num = 206)
{
   ...
   hAngle->Draw("");
   hAngle->SetTitle("Angle Between Tracks From Fitting Events Outside Cut Range");
   hAngle->GetYaxis()->SetTitle("Angle Between Tracks");
   hAngle->GetYaxis()->CenterTitle(true);
   hAngle->GetXaxis()->SetTitle("Cut Radius (mm)");
   hAngle->GetXaxis()->CenterTitle(true);
}
...
void FitTracksPlotResiduals(int cutRadius = 0, int tpc_run_num = 206)
{
   ...
   for (int i = 0; i < 2; ++i) {
      ...
      for (int i1 = 0; i1 < checkcountTrack1 + 1; ++i1) {
         ...
         auto d1 =
            sqrt((x1 - checktrack1x[i1]) * (x1 - checktrack1x[i1]) + (y1 - checktrack1y[i1]) * (y1 - checktrack1y[i1]));
      ...
      for (int i2 = 0; i2 < checkcountTrack2 + 1; ++i2) {
         ...
         auto d2 =
            sqrt((x2 - checktrack2x[i2]) * (x2 - checktrack2x[i2]) + (y2 - checktrack2y[i2]) * (y2 - checktrack2y[i2]));
   ...
   TCanvas *c = new TCanvas("c1");
   plotResiduals1->Draw("AP*");
   plotResiduals1->SetTitle("Residuals for Track 1");
   plotResiduals1->GetYaxis()->SetTitle("Residuals of User Fit (mm)");
   plotResiduals1->GetYaxis()->CenterTitle(true);
   plotResiduals1->GetXaxis()->SetTitle("Beam Z (mm)");
   plotResiduals1->GetXaxis()->CenterTitle(true);

   TCanvas *c2 = new TCanvas("c2");
   plotResiduals2->Draw("AP*");
   plotResiduals2->SetTitle("Residuals for Track 2");
   plotResiduals2->GetYaxis()->SetTitle("Residuals of User Fit (mm)");
   plotResiduals2->GetYaxis()->CenterTitle(true);
   plotResiduals2->GetXaxis()->SetTitle("Beam Z (mm)");
   plotResiduals2->GetXaxis()->CenterTitle(true);
}
...
void FitTracksPlotDistortion(int cutRadius = 200, int tpc_run_num = 206)
{
   ...
   for (int i = 0; i < 1; ++i) {
      ...
      for (int i1 = 0; i1 < checkcountTrack1 + 1; ++i1) {
         ...
         auto x1 = (f1x->GetParameter(0) + (f1x->GetParameter(1)) * (checktrack1z[i1]));
         auto y1 = (f1y->GetParameter(0) + (f1y->GetParameter(1)) * (checktrack1z[i1]));
         auto d1 = (((checktrack1x[i1]) * (checktrack1x[i1]) + (checktrack1y[i1]) * (checktrack1y[i1])) -
                    ((x1) * (x1) + (y1) * (y1))) /
                   1000000;
      ...
      for (int i2 = 0; i2 < checkcountTrack2 + 1; ++i2) {
         ...
         auto x2 = (f2x->GetParameter(0) + (f2x->GetParameter(1)) * (checktrack2z[i2]));
         auto y2 = (f2y->GetParameter(0) + (f2y->GetParameter(1)) * (checktrack2z[i2]));
         auto d2 = (((checktrack2x[i2]) * (checktrack2x[i2]) + (checktrack2y[i2]) * (checktrack2y[i2])) -
                    ((x2) * (x2) + (y2) * (y2))) /
                   1000000;
   ...
   TCanvas *c = new TCanvas("c1");
   plotDistortion1->Draw("AP*");
   plotDistortion1->SetTitle("Distortion for Track 1");
   plotDistortion1->GetYaxis()->SetTitle("Radial Drift Distortion (m^2)");
   plotDistortion1->GetYaxis()->CenterTitle(true);
   plotDistortion1->GetXaxis()->SetTitle("Electron Drift Distance in Z (m)");
   plotDistortion1->GetXaxis()->CenterTitle(true);

   TCanvas *c2 = new TCanvas("c2");
   plotDistortion2->Draw("AP*");
   plotDistortion2->SetTitle("Distortion for Track 2");
   plotDistortion2->GetYaxis()->SetTitle("Radial Drift Distortion (m^2)");
   plotDistortion2->GetYaxis()->CenterTitle(true);
   plotDistortion2->GetXaxis()->SetTitle("Electron Drift Distance in Z (m)");
   plotDistortion2->GetXaxis()->CenterTitle(true);
}
