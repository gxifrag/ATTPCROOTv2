...
void visualizeTriplets(pcl::visualization::PCLVisualizer &viewer, pcl::PointCloud<pcl::PointXYZI>::Ptr cloud,
                       std::vector<hc::triplet> const &triplets)
{
   size_t lineId = 0;

   for (size_t tripletIndex = 0; tripletIndex < triplets.size(); ++tripletIndex) {
...
void visualizeClusterAsMst(pcl::visualization::PCLVisualizer &viewer, pcl::PointCloud<pcl::PointXYZI>::Ptr cloud,
...
@return the clusters
*/
Cluster use_hc(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, std::vector<hc::triplet> triplets, float scale, float cdist,
...
void colorByIndex(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_rgb)
{
   ...
   size_t index = 0;
   for (pcl::PointCloud<pcl::PointXYZRGB>::iterator it = cloud_rgb->begin(); it != cloud_rgb->end(); ++it) {
...
void colorByIntensity(pcl::PointCloud<pcl::PointXYZI>::ConstPtr cloud, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_rgb,
...
void colorByCluster(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_rgb,
                    Cluster const cluster, std::vector<rgb_t> const cluster_rgb)
{
   cloud_rgb->clear();
   pcl::copyPointCloud(*cloud, *cloud_rgb);
...
void clustersToGnuplot(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, Cluster const cluster, std::vector<rgb_t> cluster_rgb)
{
   ...
   for (size_t clusterIndex = 0; clusterIndex < clusters.size(); ++clusterIndex) {
      ...
      header << " '-' with points lc '#" << std::hex << rgb_hex << "' title 'track " << clusterIndex << "',";
      for (size_t i = 0; i < pointIndices->indices.size(); ++i) {
         ...
         for (std::vector<pcl::PointXYZ, Eigen::aligned_allocator<pcl::PointXYZ>>::iterator it = points.end();
              it != points.begin(); --it) {
   ...
   for (std::vector<pcl::PointXYZ, Eigen::aligned_allocator<pcl::PointXYZ>>::iterator it = points.begin();
        it != points.end(); ++it) {
...
@return vector of mean squared distances of every point in the cloud
*/
void debugGnuplot(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_smooth, pcl::PointCloud<pcl::PointXYZI>::Ptr cloud,
...
#ifdef TIME
   time1 = clock() - tstart;
   std::cerr << "file loading time: " << time1 / CLOCKS_PER_SEC << "s" << std::endl;
#endif
...
// smoothen cloud
   ...
   cloud_xyzti_smooth = smoothenCloud(cloud_xyzti, opt_params.smoothRadius); // radius
   ...
   tstart = clock();
#endif
   triplets = hc::generateTriplets(cloud_xyzti_smooth, opt_params.genTripletsNnKandidates, opt_params.genTripletsNBest,
                                   opt_params.genTripletsMaxError);
#ifdef TIME
   time1 = clock() - tstart;
   std::cerr << "gen triplet time: " << time1 / CLOCKS_PER_SEC << "s" << std::endl;
#endif
   ...
   */
   cluster = use_hc(cloud_xyzti_smooth, triplets, opt_params.cloudScaleModifier, opt_params.bestClusterDistanceDelta,
                    opt_params.cleanupMinTriplets, opt_verbose);
#ifdef TIME
   time1 = clock() - tstart;
   std::cerr << "clustering time: " << time1 / CLOCKS_PER_SEC << "s" << std::endl;
#endif
   ...
   if (gnuplot) {
      ...
      if (opt_verbose > 2) {
         viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_OPACITY, 0.75, "cloud");
      }
