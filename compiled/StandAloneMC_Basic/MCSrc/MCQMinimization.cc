...
Bool_t MCQMinimization::MinimizeOptMapAmp(Double_t *parameter, ATEvent *event, TH2Poly *hPadPlane, multiarray PadCoord)
{
 ...
 for (size_t i = 0; i < npadtotal; i++) {
   ...
   for (imc1 = 0; imc1 < imc1max; imc1++) {
      ...
      for (imc2 = 0; imc2 < imc2max; imc2++) {
         ...
         if (CHi2fit < Chimin) {
            ...
            icontrol = 3;
            // MCvar(icontrol,iconvar,x0MC, y0MC, z0MC, aMC,phiMC, rangeMC, x0MCv, y0MCv,z0MCv, aMCv, phiMCv, rangeMCv);
            // // take the last value as new starting
   ...
   QMCsim(parameter, Qsim, zsimq, QMCtotal, x0MC, y0MC, z0MC, aMC, phiMC, Bmin, dens, romin, e0sm,
          PadCoord); // simulation with Chimin parameters
   ...
   // Chi2MC(Qtrack,ztrackq,Qtracktotal,Qsim,zsimq,QMCtotal,CHi2fit,sigmaq,sigmaz);   //Chi2 to compare track and MC for
   // Chimin parameters
   ...
   std::cout << " Vertex Position (Backward extrapolation) - X : " << x0MC << " - Y : " << y0MC << " - Z : " << z0MC
             << std::endl;
...
void MCQMinimization::MCvar(double *parameter, int &modevar, int &iconvar, double &x0MC, double &y0MC, double &z0MC,
...
void MCQMinimization::QMCsim(double *parameter, double *Qsim, double *zsimq, double &QMCtotal, double x0MC, double y0MC,
                             double z0MC, double aMC, double phiMC, double Bmin, double dens, double romin,
                             double &e0sm, multiarray PadCoord)
{
   ...
   for (Int_t k = 0; k < integrationmax; k++) {
      ...
      if (iz1 == 1) {
         ...
         sloss = 6.98 * (1. / TMath::Power(c0, 0.83)) * (1. / (20. + 1.6 / TMath::Power(c0, 1.3))) +
                 0.45 * TMath::Exp(-55. * TMath::Power((c0 - 0.025), 2)); // expression modified June 14 2016
      ...
      */
      ...
      for (isig = 0; isig < 4; ++isig) {
         ...
         for (iphi = 0; iphi < 8; ++iphi) {
            ...
            // std::cout<<"integration :  "<<integration<<" X_str : "<<X_str<<" Y_str : "<<Y_str<<" Q_str :
            // "<<Q_str<<std::endl;
            ...
            if (pBin > 0) {
               ...
               // std::cout<<"X_str : "<<X_str<<" Y_str : "<<Y_str<<"  Pad Number : "<<pBin+1<<" Bin content :
               // "<<qBin<<"  table "<<Qsim[pBin]<<std::endl;
            } // end if
         } // phi angle
   ...
   for (Int_t i = 0; i < npadtotal; i++) {
      ...
      if (Qsim[i] > tracksimthreshold) // threshold after renormalization
      {
         ...
         // fout1<<"  test of coordinates"<<xhelp<<"  1 "<<xhelp1<<" 2  "<<xhelp2<<"   y "<<yhelp<<"  "<<"   y1
         // "<<yhelp1<<endl;
         ivalidsim++;
         double help;
         help = zsimq[i];
...
void MCQMinimization::Chi2MC(double *Qtrack, double *ztrackq, double &Qtracktotal, double *Qsim, double *zsimq,
                             double &QMCtotal, double &Chi2fit, double &sigmaq, double &sigmaz)
{
   ...
   for (i = 0; i < npadtotal; i++) {
      ...
      if (Qsim[i] > qsimthreshold) {
         ...
         if (Qtrack[i] > qtrackthreshold) {
            ...
            Chi2fitq += sigq * sigq;
            Chi2fitz += sigz * sigz;
            // std:: cout<< " sim   "<< Qsim[i] <<"  track  " << Qtrack[i]<< "  Chi2z " << Chi2fitz<<"  sigz  "<<sigz<<"
            // zsim "<<zsimq[i]<< " ztrack= "<<ztrackq[i]<<" npoints  " <<npoints<<endl;
         }
      }
   }
   // normalized
   ...
   // Chi2number<<" npoints"<< npoints <<endl; std:: cout<< " chi2fitnorm**3  "<<Chi2fit<<"  Chi2q  " << Chi2fitq<<"
   // Chi2z " << Chi2fitz<<" npoints "<< npoints <<endl;
}
