...
__global__ void calcInner(int i, curandState *state, Double_t *parameter)
{
   ...
   if (idx < 400) {
      ...
      Double_t dens = 0.06363 * 18 * (1 + step8 * (curand_uniform(&state[idx]) - 0.5)) /
      ...
      for (Int_t k = 0; k < iterationmax; k++) {
         ...
         if (iz1 == 1) {
            ...
            if (m == 2)
               c0 = c0 / 2.0;
            sloss = 6.98 * (1. / pow(c0, 0.83)) * (1. / (20. + 1.6 / pow(c0, 1.3))) +
                    0.2 * exp(-30. * pow((c0 - 0.1), 2.0)); // Old expression
         ...
         if (iz1 == 2) {
            c0 = ekin;
            sloss =
               11.95 * (1. / pow(c0, 0.83)) * (1. / (2.5 + 1.6 / pow(c0, 1.5))) + 0.05 * exp(pow(-(c0 - 0.5), 2.0));
         }
